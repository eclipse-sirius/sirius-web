...
test.describe('domain explorer - tree item palette', () => {
  ...
  test('when a tool that redirect the user clicked, then the tool redirects the user', async ({ page, context }) => {
    const playwrightExplorer = new PlaywrightExplorer(page);
    const helpUrl = 'https://eclipse.dev/sirius/sirius-web.html';

    // Intercept at context level to also handle the popup's initial navigation.
    await context.route(helpUrl, async (route) => {
      await route.fulfill({ status: 200, contentType: 'text/html', body: '<html><body>Help</body></html>' });
    });
    ...
    await expect(newPage).toHaveURL(helpUrl);
  });
});
