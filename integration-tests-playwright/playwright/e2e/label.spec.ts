...
test.describe('diagram - label', () => {
  ...
  test('when the reset label size tool is triggered on an outside label, then label size is reset', async ({
    ...
    await expect(page.getByTestId('reset-label-size')).toBeAttached();
    await page.getByTestId('reset-label-size').click();
    ...
    await entity1Node.openPalette();
    await expect(page.getByTestId('reset-label-size')).not.toBeAttached();
  });
