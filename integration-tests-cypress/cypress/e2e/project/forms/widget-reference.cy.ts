...
const createFormWithWidgetRef = (domainType: string, name: string, reference: string) => {
  ...
  explorer.createObject('ReferenceWidgetClearButtonDescription', 'body-ChangeContext');
  details.getTextField('Expression').should('exist');
  details.getTextField('Expression').clear();
  details.getTextField('Expression').type('aql:self.defaultClearReference(referenceName)');
