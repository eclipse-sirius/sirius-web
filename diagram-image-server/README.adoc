...
2. Launch the script: `node index.mjs -s https://your-sirius-web-server`
...
[IMPORTANT]
----
Note the `https` in the `https://your-sirius-web-server` URL above.
The Sirius Web frontend must execute in a [Secure Context](https://developer.mozilla.org/en-US/docs/Web/Security/Defenses/Secure_Contexts) to work correctly.
When testing locally, you can use plain HTTP if you target `localhost`, but for any real deployment (including inside a docker compose setup), the Sirius Web frontend *must* be accessed by HTTPS.
Otherwise you will get errors like _TypeError: crypto.randomUUID is not a function_ and the frontend will not be able to communicate with the backend.
----
