...
Existing code following this pattern: `if (myEvent.causedBy() instanceof InvokeSingleClickOnDiagramElementToolInput)` needs to navigate one step further in the causality chain to perform the same check. 
...
`ChangeDescription` has been updated to accept an `ICause` as parameter instead of just `IInput`.
