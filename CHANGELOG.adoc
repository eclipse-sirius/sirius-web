...
- https://github.com/eclipse-sirius/sirius-web/issues/4652[#4652] [sirius-web] Add support for project to library dependencies
Project can have dependencies to libraries, which are loaded along the project's semantic data.
Loading a library in the editing context loads all the libraries it transitively depends on.
A new predicate `IReadOnlyObjectPredicate` has been added with a default implementation that returns `true` if the object is in a library.
