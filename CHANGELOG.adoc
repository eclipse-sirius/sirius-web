...
- [ADR-239] Make reference widget more configurable
...
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [sirius-web] Reduce the dependency of the project semantic data bounded context on `sirius-web-domain` by adding a dedicated configuration module, `sirius-web-projects-semanticdata-infrastructure`, for its internationalization support.
...
`clearHandlerProvider` has been removed from `ReferenceWidgetDescription`.
`referenceNameProvider` has been added to `ReferenceWidgetDescription` to provide the name of the reference that will be edited.
A new API has been added to replace `clearHandlerProvider`: `IReferenceWidgetClearHandler`, a candidate to handle the clear event; and `IDefaultReferenceWidgetClearHandler` the default implementation used when no candidate is able to handle the clear event.
An implementation of `IDefaultReferenceWidgetClearHandler`, `ReferenceWidgetDefaultClearHandler`, is by default provided by Sirius Web.
...
- [releng] Switch to https://github.com/TNG/ArchUnit/releases/tag/v1.5.0[ArchUnit 1.5.0] and https://github.com/xmolecules/jmolecules/releases/tag/2.0.1[jmolecules 2.0.1] for our backend architecture tests.
- [releng] Switch to https://github.com/eclipse-sirius/sirius-emf-json[EMFJson 2.5.5]
- [releng] Switch to https://github.com/apollographql/apollo-client/releases/tag/v3.14.1[apollo-client 3.14.1]
...
- https://github.com/eclipse-sirius/sirius-web/issues/6992[#6992] [diagram] Add missing palette for the views explorer representation
...
- https://github.com/eclipse-sirius/sirius-web/issues/6950[#6950] [diagram] Add support for node style customization.
Implement `INodeStyleCustomizer` to provide a node style customization.
If many node style customizations can apply to an element, they are all applied.
- https://github.com/eclipse-sirius/sirius-web/issues/6964[#6964] [sirius-web] Add the appearance project settings view.
The feature can be tested by using the configuration property `sirius.web.style.customization.enabled` with the value `true`.
It also activates some node style customizations, used for tests, for flow diagrams if the label of an object contains either "I'm Blue" or "da be di da be dai".
