...
- https://github.com/eclipse-sirius/sirius-web/issues/2809[#2809] [forms] The _Reference_ widget can now provide an optional `modelBrowserId`.
It can be used, in conjunction with `IModelBrowserMetadataProvider` on the backend, to use a custom `TreeDescription` to display the possible candidates in the modals used to edit the reference on the frontend.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5556[#5556] [sirius-web] In the onboard area, update `NewRepresentationArea`'s entries labels from `defaultName` to `defaultName: diagramDescriptionName` to make it more clear which representation description will be used when creating a new representation.
