...
It is now possible to add `KeyBinding` instances to any tool in a diagram (see `NoteToolBuilder#keyBindings`), and specify the keys to press to trigger the tool.
Note that for the moment only `SingleClickOnDiagramElementTool` can be triggered with key bindings, although it is possible to define key bindings for any tool.
The key bindings that can be triggered at a given time are computed from the palette of the selected diagram elements.
For example, the the tools contained in a given `NodeDescription` can only be triggered via key bindings if the selected diagram element is a node with the given `NodeDescription`.
The key bindings are visible in the tooltip of the tools in the palette.
