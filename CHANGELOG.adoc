...
- [ADR-191] Diagram elements appearance management
...
- https://github.com/eclipse-sirius/sirius-web/issues/4756[#4756] [diagram] The `NodeTypeContextValue` expects a new field `nodeTypeAppearanceSectionContributions` used to contribute content for the palette appearance section
- https://github.com/eclipse-sirius/sirius-web/issues/4756[#4756] [diagram] Concerning frontend renderer types, The `NodeData` type expects a `nodeAppearanceData` field, the `InsideLabel` and `OutsideLabel` expect an `appearanceData` field. These new fields are used to store the effective and customized styles. This is only breaking for pre-existing custom nodes.
...
+ https://github.com/eclipse-sirius/sirius-web/issues/4756[#4756] [diagram] Add an appearance section for the diagram elements palette
