...
- https://github.com/eclipse-sirius/sirius-web/issues/3305[#3305] `IExplorerServices#getDefaultElements` has been removed. Use `DefaultContentService#getContents` instead by passing the `IEditingContext`.
*Note* that `DefaultContentService#getContents` will not return a sorted list as it was before by `IExplorerServices#getDefaultElements`.
