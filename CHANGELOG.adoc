...
- https://github.com/eclipse-sirius/sirius-web/issues/5628[#5628] [diagram] During custom nodes layout, all the newly created nodes are now used and not only the first one.
This mean the `INodeLayoutHandler` interface change from `newlyAddedNode: Node<NodeData, DiagramNodeType> | undefined` to `newlyAddedNode: Node<NodeData, DiagramNodeType>[]`.
