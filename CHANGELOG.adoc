...
- https://github.com/eclipse-sirius/sirius-web/issues/5509[#5509] [diagram] The GraphQL field `connectorTools` in `DiagramDescription` has been changed from `connectorTools(sourceDiagramElementId: ID!, targetDiagramElementId: ID!): [Tool!]!` to `connectorTools(sourceDiagramElementId: ID!): [ConnectorTool!]!`.
* This can now be used to retrieve both the available connection tools for a given source node and the available target candidates for a given tool.
- https://github.com/eclipse-sirius/sirius-web/issues/5514[#5514] [diagram] The GraphQL field palette no longer return `SingleClickOnTwoDiagramElementsTool` tools, you should use the `connectorTools` field in order to retrieve the connector tools.
...
- https://github.com/eclipse-sirius/sirius-web/issues/5509[#5509] [diagram] Improve `connectorTools` field in order to get all the information about a connector tool in one request
- https://github.com/eclipse-sirius/sirius-web/issues/5534[#5534] [diagram] Export Palette component to a dedicated package
