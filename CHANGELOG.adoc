...
- https://github.com/eclipse-sirius/sirius-web/issues/5863[#5863] `selectedNode` and `selectedEdge` AQL variables are deprecated, you can use `selectedNodes` and `selectedEdges`.

`IPaletteToolsProvider` methods signature have been changed to support group palettes : 
...
https://github.com/eclipse-sirius/sirius-web/issues/5782[#5782] [diagram] Add the concept of `GroupPalette` in the view dsl and the API `ISingleClickOnMultipleDiagramElementHandler` to execute a group tool.
When executing a group tool, self is list of the selected element.
- https://github.com/eclipse-sirius/sirius-web/issues/5863[#5863] Add `precondition` expression to `toolSection`.
Add `toolSections` and `reusedToolSections` to groupPalette in the view dsl.
When executing a tool from a `reusedToolSection`, it will be executed for each `diagramElementId` used to execute the tool, each time self will be one of the diagram element, the order is preserved.
We're introducing the `selectedNodes` and `selectedEdges` variable that will contain the selected diagram element.
These variable are also available on non group tools.
