...
- https://github.com/eclipse-sirius/sirius-web/issues/5842[#5842] [diagram] `IConvertEngine` interface has been changed, `convertNodes` now uses `nodeLookup`.
The node custom converters must be updated, an example with these changes is available with `EllipseNodeConverter`.
