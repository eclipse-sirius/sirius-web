...
- https://github.com/eclipse-sirius/sirius-web/issues/5147[#5147] [sirius-web] The `EditingContextSnapshotService` now handles semantic data's dependencies.
The method `void restore(IEditingContext, IEditingContextSnapshot)` has been updated to `void restore(IEditingContext, IEditingContextSnapshot, ICause cause)` to reflect this change.
...
* Number of references that will be broken by the update (new proxies that won't be resolved after the update)
* Number of references that will be fixed by the update (old proxies that will be resolved after the update)
