...
- https://github.com/eclipse-sirius/sirius-web/issues/2503[#2503] Change descriptions name to id and add a documentation field
  * `RepresentationDescription#name` has been renamed to `id` (API changes: `getName()`/`setName()` -> `getId()`/`setId()`, feature constant `REPRESENTATION_DESCRIPTION__NAME` -> `REPRESENTATION_DESCRIPTION__ID`). Generated models and Java APIs have been updated; downstream code referencing the old name feature or accessing descriptions by name must be adapted to use `id`.
  * Updated generated model types: several `EdgeStyle` usages have been renamed to `EdgeStyleDescription` and conditional types adjusted accordingly.
