...
- https://github.com/eclipse-sirius/sirius-components/issues/1584[#1584] [diagram] Leaf nodes (nodes with no children) which have an explicitly set size (either set by the end-user or by the node's definition) now keep heir size during an _Arrange All_ operation.
