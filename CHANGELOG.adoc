...
- https://github.com/eclipse-sirius/sirius-web/issues/6933[#6933] [palette] Ensure `ViewPaletteProvider` and `ViewConnectorPaletteProvider` define consistent `self` variables.
`self` is now the selected element when opening the palette, and the source element when retrieving the connectors.
This ensures that an edge defined with a precondition is correctly evaluated in both scenarios.
