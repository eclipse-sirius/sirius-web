...
Existing code inspecting causes (via `cause.causedBy()`) should now rely on `ICausalityChainVisitor#findFirstCauseOfType`, which allows to navigate causality chains in a generic way. 
