...
- https://github.com/eclipse-sirius/sirius-components/issues/1288[#1288] [core] The `representationDescriptions` GraphQL has been renamed into `representationCreationDescriptions` and now takes an `objectId: ID` as argument instead of just the object's `kind: ID`.
...
- https://github.com/eclipse-sirius/sirius-components/issues/1289[#1289] [core] The `representationCreationDescriptions` GraphQL query now returns a new attribute named `defaultName` in addition to the `label` and `id`. For view-based representations and , the `defaultName` is computed from the `RepresentationDescription`'s `titleExpression`.
