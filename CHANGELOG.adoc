...
As such, the variables `droppedNode` and `droppedElement` which are exposed in the body of such tools and which only supported a single node have been replaced with `droppedNodes` and `droppedElements` representing all the nodes (and corresponding semantic elements) dropped at once.
