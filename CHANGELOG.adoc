...
- [ADR-124] Improve custom node API to reduce duplication
...
----
- https://github.com/eclipse-sirius/sirius-web/issues/2628[#2628] [diagram] Improve the API available for custom nodes
- https://github.com/eclipse-sirius/sirius-web/issues/2831[#2831] [diagram] Add inside label location to the node data.
Custom nodes based on the `ListNodeConverter` and `RectangleNodeConverter` should handle the inside label location directly in the reactflow node.

=== Dependency update

- [releng] The frontend now depends on @ObeoNetwork/gantt-task-react v0.3.12
...
- https://github.com/eclipse-sirius/sirius-web/issues/2484[#2484] [core] [diagram] Split the frontend label into edge label, inside label and outside label.
Add the data structure for outside labels.
