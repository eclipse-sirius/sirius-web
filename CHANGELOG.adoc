...
- https://github.com/eclipse-sirius/sirius-web/issues/5295[#5295] [core] Add an API to contribute undo redo handlers for graphical events (TODO Move)
- https://github.com/eclipse-sirius/sirius-web/issues/5300[#5300] [diagram] Add support for undo redo node position changes 
...
- https://github.com/eclipse-sirius/sirius-web/issues/5148[#5148] [core] Add `IRepresentationEventProcessorChangeListener` in order to handle onChange changeDescription from the `EditingContextEventProcessor`
- https://github.com/eclipse-sirius/sirius-web/issues/5285[#5285] [core] Add `IRepresentationEventProcessorGetter` in order to handle `acquireRepresentationEventProcessor(String representationId, IInput input)` method from the `EditingContextEventProcessor`.
Add `IEditingContextEventProcessorHandler` in order to handle `doHandle(payloadSink, input)` method from the `EditingContextEventProcessor`
