...
The cards are not contributed to the `createProjectAreaCardExtensionPoint` extension point anymore.
...
Another consequences is that the `projectTemplates` graphql query now requires the `withDefault` parameter.
...
This purpose can be achieved with the `canRename` capabilities.
...
This purpose can be achieve with the `canEdit` project capabilities by contributing a `ICapabilityVoter`:
[source,java]
----
@Service
public class canEditProjectCapabilityVoter implements ICapabilityVoter {
  ...
  public CapabilityVote vote(String type, String identifier, String capability) {
    if ("Project".equals(type) && "canEdit".equals(capability) && identifier != null) {
...
- https://github.com/eclipse-sirius/sirius-web/issues/5126[#5126] [core] Add backend support for capabilities.
+ Providing one or many ICapabilityVoter, downstream projects will be able to remove some frontent components to prevent their users to use them.
