...
Fields `dialogDescriptionId`, `withImpactAnalysis` and `keyBindings` from `SingleClickOnTwoDiagramElementTool` are now part of the `ITool` interface.
Palette `types` from the react component and the GraphQL queries are now exported from the palette package.
