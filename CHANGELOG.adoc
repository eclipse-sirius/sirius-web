...
- https://github.com/eclipse-sirius/sirius-web/issues/3985[#3985] [core] Add first support for the dark theme.
+ Based on the `prefers-color-scheme` media query, it activates the dark theme based on the browser preference for firefox or, the OS preference for chrome.
+ Styles in representation descriptions does not leverage on the `color scheme` yet.
