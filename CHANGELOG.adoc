...
- https://github.com/eclipse-sirius/sirius-web/issues/4143[#4143] [core] Add postRefresh method in IRepresentationEventProcessor
- https://github.com/eclipse-sirius/sirius-web/issues/4106[#4106] [portal] Add undo/redo for portal representation
- https://github.com/eclipse-sirius/sirius-web/issues/4131[#4131] [diagram] Add undo/redo for diagram representation

=== Improvements

- https://github.com/eclipse-sirius/sirius-web/issues/4145[#4145] [core] Add IChangeDescriptionListener to allow subscribing to change made in an editing context.
- https://github.com/eclipse-sirius/sirius-web/issues/4145[#4145] [core] Split the _editing context event processor_.
** The `RepresentationEventProcessorRegistry` which handles the lifecyle of representations in the editing context.
** The `EditingContextExecutor` encapsulate editing context event handlers with _input pre-processors_ and _input post-processors_, all in their own thread.
** The `ComposedEditingContextEventHandler` which dispatches editing context events to the right handler.
** The `EditingContextManager` which is used to react to change made and thus, persist them is need.
...
- [ADR-178] Split editing context event processor responsibility
...
https://github.com/eclipse-sirius/sirius-web/issues/4145[#4145] [core] Remove `ChangeKind#REPRESENTATION_TO_RENAME`.
+ The rename of a representation needs to be done anywhere an object that can be rename represents a representation.
+ An example can be found in `RenameRepresentationTreeItemHandler`.
+ You will have nothing to do if you are based on sirius web application.
