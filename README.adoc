...
1. Install https://www.docker.com/[Docker] if you do not have it already.
2. Download the https://raw.githubusercontent.com/eclipse-sirius/sirius-web/refs/heads/master/docker-compose.yml[`docker-compose.yml`] file from the root of the Sirius Web git repository.
3. Open a terminal in the directory where you put the `docker-compose.yml` file, and issue `docker compose up`.

Docker will automatically download and start both the database (PostgreSQL) image and the latest version of Sirius Web published https://hub.docker.com/r/eclipsesirius/sirius-web[on Docker Hub].

WARNING: This may take a while the first time you run this as Docker will first pull the required Docker images.
...
NOTE: This way of running the application is only useful for quick testing and should not be used for actual deployments.

[#documentation]
== Documentation

For further information not covered in this README, including a user manual, refer to https://eclipse-sirius.github.io/sirius-web/sirius-web/index.html[the full Sirius Web documentation].
...
** LTS versions of https://nodejs.org/[Node and NPM]: in particular, Node >= 24.20.0 is required along with npm >= 11.19.0.
