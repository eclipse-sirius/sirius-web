  ...
  context('Given a view with a node tool with impact analysis', () => {
    context('When we create a new instance project', () => {
      ...
      it('Then the tool trigger the impact analysis dialog with the good report', () => {
        ...
        impactAnalysis.getNbElementModified().should('contain', 'Number of items modified: 2');
        impactAnalysis.getNbElementDeleted().should('contain', 'Number of items deleted: 0');
        impactAnalysis.getNbElementCreated().should('contain', 'Number of items added: 1');
      });

      it('Then after impact analysis it is possible to cancel the tool execution', () => {
        const explorer = new Explorer();
        const diagram = new Diagram();
        const details = new Details();
        const impactAnalysis = new ImpactAnalysis();
        ...
        diagram.getPalette().findByTestId('tool-Tool with impact analysis').click();
        impactAnalysis.getImpactAnalysisDialog().should('exist');
        impactAnalysis.cancelExecution();
        diagram.getNodes('Impact analysis diagram', 'InitialName').should('exist');
        diagram.getNodes('Impact analysis diagram', 'newName').should('not.exist');
      });
