    ...
    it('auto completion is working when pressing ctrl+space', () => {
      ...
      details
        ...
        .then((actualValue) => {
          ...
          details.getTextField('Domain Type').type('{ctrl}', { release: false });
          details.getTextField('Domain Type').trigger('keydown', { keyCode: 32, which: 32 });
          cy.getByTestId('completion-proposals').should('exist');
          cy.getByTestId('completion-proposals')
            .contains(actualValue as string)
            .click();
          details.getTextField('Domain Type').contains(actualValue as string);
        });

      // const firstElementText : string = cy.getByTestId('completion-proposals').children[0];
    });
  });
});
