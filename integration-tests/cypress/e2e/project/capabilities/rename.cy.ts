...
const projectName = 'Cypress - Disabled Rename Project';
  ...
  context('Given a project with a specific name', () => {
    ...
    context('When we try to rename the project from the project browser view', () => {
      ...
      it('Then the rename button should be disabled', () => {
        new Projects().getActionMenu(projectName).getRenameButton().should('have.class', 'Mui-disabled');
      });
    });
    ...
    context('When we try to rename the project from the workbench', () => {
      ...
      it('Then the rename button should be disabled', () => {
        new Project().getProjectNavigationBar(projectName).getRenameButton().should('have.class', 'Mui-disabled');
      });
    });
  });
});
