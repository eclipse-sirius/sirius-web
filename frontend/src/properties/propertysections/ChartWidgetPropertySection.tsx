...
const BarChartComponent = ({ width, height, chart }: BarChartProps) => {
  ...
  useEffect(() => {
    ...
    if (d3Container.current && chart) {
      const { entries: data, label: yLabel } = chart as BarChart;
      const x = (d) => d.key;
      const y = (d) => d.value;
...
const PieChartComponent = ({ width, height, chart }: PieChartProps) => {
  const d3Container = useRef<SVGSVGElement>(null);
  useEffect(() => {
    const { entries: data } = chart as PieChart;
    ...
    const label = (d) => {
      ...
      var value: string[] = d.endAngle - d.startAngle > 0.25 ? lines : lines.slice(0, 1);
      return value.map((v) => {
        if (v.length > 13) {
