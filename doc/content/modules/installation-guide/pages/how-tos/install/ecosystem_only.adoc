...
[source, bash]
----
docker run -p 5432:5432 --name sirius-web-postgres -e POSTGRES_USER=dbuser -e POSTGRES_PASSWORD=dbpwd -e POSTGRES_DB=sirius-web-db -d postgres
----
====
...
java -jar sirius-web-YYY.M.X.jar \
          --spring.datasource.url=jdbc:postgresql://localhost:5432/{product-artefact}-db \
