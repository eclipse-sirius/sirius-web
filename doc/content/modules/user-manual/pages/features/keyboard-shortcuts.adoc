...
To delete a given element, user can select the element and the press the `Del` key in the supported representation editor.
...
User can press `F2` key or start typing the new value directly to efficiently update an element name in the supported representation editor.
...
Explore further by referring to the following how-tos for xref:user-manual:hands-on/how-tos/explorer-view.adoc#explorer-filter[Filter elements].