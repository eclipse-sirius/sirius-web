...
You focus on modeling; the available tools and rules are already configured for you.
...
a|image::global-palette-manage-selection.png[Manage selection]
| Manage selection
| Select or unselect elements based on their type or hierarchy on the diagram.
|===
...
When a rectangular selection starts on a node, that node and its containers are excluded from the selection.
Start the selection outside nodes to include containers.
====

[NOTE]
====
When selecting multiple elements, you can observe that the last selected element is highlighted with a different angle-handle color.
Most of the layout tools are relative to this last selected element.
====
...
a|image::contextual-palette-pin.png[Pin or unpin]
| *Pin / Unpin*
| Pins all selected nodes so they stay in place when the diagram is rearranged.
When all selected nodes are already pinned, the tool becomes *Unpin* and unpins them.

a|image::contextual-palette-align-horizontal.png[Align horizontally]
| *Align horizontally*
| Aligns selected elements horizontally to the left, center or right, relative to the last selected element.

a|image::contextual-palette-align-vertical.png[Align vertically]
| *Align vertically*
| Aligns selected elements vertically to the top, middle or bottom, relative to the last selected element.
