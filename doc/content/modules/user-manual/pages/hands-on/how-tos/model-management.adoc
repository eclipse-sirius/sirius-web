...
{product} supports the upload of XMI and JSON files.
...
Ensure that your models are exported using the xref:user-manual:hands-on/how-tos/model-management.adoc#download-model[Download Model] action to enable successful re-uploading to {product}.
====

To upload a model from a file previously exported, follow these steps:

. Go to the {project-workbench}.
. Open the {explorer-view}.
. Click the `Upload` button at the top of the {explorer-view} in the toolbar.
...
. Click the {explorer-menu} next to the model in the {explorer-view}.
...
. Select the model in the {explorer-view}.
...
. To navigate through the details of a semantic element, use the {explorer-view} to navigate through the model and locate the needed element.
. Select the element in the {explorer-view}.
. On the right panel, access the {details-view}.
...
. Click the {explorer-menu} next to the element in the {explorer-view}.
. Select the `Rename` option.
. Update the name in the editable field.
. Click `Rename`.
...
. Select the `Edit` tool in the {diagram-contextual-palette}.
. Validate the changes by pressing the `RETURN` key or clicking elsewhere.
...
. Select the element in the representation.
. Click the `Delete` tool in the {diagram-contextual-palette} or press the `DEL` key on your keyboard.
