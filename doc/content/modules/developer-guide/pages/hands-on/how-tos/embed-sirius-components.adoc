...
=== Navigate a change causality chain

`ChangeDescription#getCause()` returns the cause associated with the change, which may wrap the original `IInput` in a causality chain.
When a handler or event listener needs to retrieve a specific cause, use `ICausalityChainVisitor#findFirstCauseOfType` instead of traversing `ICause#causedBy()` directly:

```java
Optional<MyInput> optionalInput = causalityChainVisitor.findFirstCauseOfType(changeDescription.getCause(), MyInput.class);
```
