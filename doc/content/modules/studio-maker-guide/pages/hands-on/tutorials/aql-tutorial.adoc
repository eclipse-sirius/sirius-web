...
.. Note that `self` changes when you run expressions inside descriptions, styles, or tools, so always confirm it before writing complex logic.
. **Add expressions to styles**
.. Pick a description and edit its label expression to use AQL (for example `aql:self.name.concat(' (', self.status, ')')`).
