...
This page explains how the tree description can be configured in {product}.
...
Tree representation descriptions describe how semantic elements become tree items, which attributes drive expandability, and which context menu entries provide actions, all using the familiar expression editor.
Tree representation descriptions describe how semantic elements become tree items, which attributes drive expandability, and which context menu entries provide actions.
...
* `CustomTreeItemContextMenuEntry`: registers contribution identifiers provided by the frontend (`contributionId`) and can optionally set `withImpactAnalysis` & `labelExpression`.

When using a `CustomTreeItemContextMenuEntry` you can reuse an existing contributed tool, some of the available ids are :

* `expand-all` : Expand all the children of the tree item
* `new-root-object` : If the tree item is a document, open a modal to create a root object
* `download-document` : If the tree item is a document, download the document
* `new-object` : If the tree item is an eobject, open a modal to create a children
* `new-representation` : If the tree item is an eobject, open a modal to create a representation
* `duplicate-object` : If the tree item is an eobject, open a modal to duplicate the object
* `duplicate-representation` : If the tree item is a representation, duplicate the representation
* `update-library` : If the tree item is a library, open a modal to update the library

Note that you have to check the type of the tree item (using a precondition for example) to avoid rendering it on a type that is not compatible with the tool).
