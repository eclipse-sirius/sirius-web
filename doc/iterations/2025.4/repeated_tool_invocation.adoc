...
If the user presses and holds the _Alt_ (_Cmd_) key, the cursor changes to [`cursor: copy`](https://developer.mozilla.org/en-US/docs/Web/CSS/cursor).
...
When selecting the target node of an edge tool, if the _Alt_ key is pressed and the corresponding palette has a "last invoked tool" in memory, it is invoked automatically withouth opening the UI.
...
3. Alice presses and holds _Alt_ (_Cmd_) and left clicks 3 times in a row on the diagram's background.
4. Each time, the same tool _TP_ is re-invoked on the diagram, creating nodes _N2_, _N3_, _N4_.

_Scenario 2 (after scenario 2):_

1. Alice presses and holds _Alt_ (_Cmd_) and left clicks on _N1_.
...
4. Alice presses and holds _Alt_ (_Cmd_) and left clicks on _N2_, _N3_, and _N4_ in quick succession.
