= ADR-209 - Add the support for backend workbench configuration
...
What we want and what we already have look similar, and we want to leverage on that
...
Apart from the graphql schema, we will provide the interface `IProjectWorkbenchConfigurationProviderService`.

[source, java]
----

public interface IProjectWorkbenchConfigurationProviderService {
