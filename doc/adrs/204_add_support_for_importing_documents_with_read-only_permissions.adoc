= ADR-204 - Add support for importing documents with read-only permissions

== Context

The upload document modal always imports resources with read/write permissions.
...
The _Upload (read/write)_ option will correspond to the actual behavior of the _Upload_ button.
...
We will add an `isReadOnly` column in the `document` database, update the corresponding bounded context, and the `ResourceToDocumentService` to persist the `isReadOnly` metadata in the database.
...
- A liquibase migration is required to support the new `isReadOnly` column in the `document` table.
