= ADR-195 - Add impact analysis for contextual menu actions of trees
...
The variables `diagramElementId` in these types will be renamed to `elementId` to decouple them from diagrams.
Some additional refactoring may be required to decouple these types from diagram-specific concepts like tools.
...
type TreeDescription implements RepresentationDescription {
  // [...]
  treeImpactAnalysisReport(toolId: ID, treeItemId: ID!, variables: [ToolVariables!]!): ImpactAnalysisReport!
}
----
