= ADR-182 Add support of row hierarchy in tables
...
In a near future, we could handle the expanded state of rows in the back-end side in order to recompute the list of semantic elements that should be displayed in the table according to current state of table.
This would imply to store/persist the expanded rows in a similar manner that it is done for tree representation.
