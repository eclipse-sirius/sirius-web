...
** `class` variable is the `EClass` of the self element
* `Title Expression`: The initial name of the representation once created
** `self` variable is the semantic element associated with the `Table`.
** `label` variable is the name of the table representation
...
All those expression have access to the following variables to perform its computation:
...
Inside these expressions, it is possible to use the following variables:

* `editingContext` the global editing context associated to the project
* `descriptionId` the id of the table description used to defined this row filter
* `representationId` the id of the representation itself
