...
/**
 * Handle "Edit Select Cell" events.
 *
 * @author arichard
 * @author lfasani
 */
@Service
public class EditSelectCellEventHandler implements ITableEventHandler {
    ...
    private final Logger logger = LoggerFactory.getLogger(EditSelectCellEventHandler.class);

    public EditSelectCellEventHandler(IObjectService objectService, ITableQueryService tableQueryService, ITableDescriptionService tableDescriptionService,
    ...
    public boolean canHandle(ITableInput tableInput) {
        return tableInput instanceof EditSelectCellInput;
    }
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, Table table, TableDescription tableDescription, ITableInput tableInput) {
        ...
        String message = this.messageService.invalidInput(tableInput.getClass().getSimpleName(), EditSelectCellInput.class.getSimpleName());
        IPayload payload = new ErrorPayload(tableInput.id(), message);

        if (tableInput instanceof EditSelectCellInput) {
            EditSelectCellInput input = (EditSelectCellInput) tableInput;
            ...
            if (optCell.isPresent()) {
                ...
                if (optLine.isPresent() && optCol.isPresent()) {
                    this.invokeEditCell(cell, optLine.get(), optCol.get(), editingContext, tableDescription, input.newValue());
    ...
    private void invokeEditCell(AbstractCell cell, Line line, Column column, IEditingContext editingContext, TableDescription tableDescription, String newValue) {
        ...
        if (optionalLineDescription.isPresent()) {
            ...
            if (optionalSelf.isPresent()) {
                ...
                lineDescription.getNewCellValueHandler().apply(variableManager, newValue);
                this.logger.debug("Edited cell with id {} to new value {}", cell.getId(), newValue); //$NON-NLS-1$
            }
        }
    }
