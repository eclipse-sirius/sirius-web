 ...
 */
@Service
public class ChangeColumnFilterEventHandler implements ITableEventHandler {

    private final ICollaborativeTableMessageService messageService;

    private final Counter counter;

    public ChangeColumnFilterEventHandler(ICollaborativeTableMessageService messageService, MeterRegistry meterRegistry) {
    ...
    public boolean canHandle(IEditingContext editingContext, ITableInput tableInput) {
        return tableInput instanceof ChangeColumnFilterInput;
    }
    ...
    public void handle(Sinks.One<IPayload> payloadSink, Sinks.Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, ITableContext tableContext, TableDescription tableDescription, ITableInput tableInput) {
        ...
        String message = this.messageService.invalidInput(tableInput.getClass().getSimpleName(), ChangeColumnFilterInput.class.getSimpleName());
        IPayload payload = new ErrorPayload(tableInput.id(), message);

        if (tableInput instanceof ChangeColumnFilterInput changeColumnFilterInput) {
            payload = new SuccessPayload(changeColumnFilterInput.id());
            changeDescription = new ChangeDescription(TableChangeKind.TABLE_COLUMNS_FILTER_CHANGE, tableInput.representationId(), tableInput, Map.of(TableChangeKind.COLUMN_FILTER_LIST_PARAM,
                    changeColumnFilterInput.columnFilters()));
        }
