 ...
 * @author Jerome Gout
 */
@QueryDataFetcher(type = "TableDescription", field = "contextMenu")
public class TableDescriptionContextMenuDataFetcher implements IDataFetcherWithFieldCoordinates<CompletableFuture<List<RowContextMenuEntry>>> {
    ...
    public TableDescriptionContextMenuDataFetcher(IEditingContextEventProcessorRegistry editingContextEventProcessorRegistry) {
    ...
    public CompletableFuture<List<RowContextMenuEntry>> get(DataFetchingEnvironment environment) throws Exception {
        ...
        var input = new RowContextMenuInput(UUID.randomUUID(), editingContextId, representationId, tableId, UUID.fromString(rowId));
