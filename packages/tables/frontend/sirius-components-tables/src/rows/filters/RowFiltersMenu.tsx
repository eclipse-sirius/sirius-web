...
export const RowFiltersMenu = ({ readOnly, rowFilters, onRowFiltersChange }: RowFiltersButtonProps) => {
  ...
  const handleToggleFilter = (filterId: string, newState: boolean) => {
    const updatedFilters = rowFilters.map((f) => (f.id === filterId ? { ...f, state: newState } : f));
    onRowFiltersChange(updatedFilters);
  };
          ...
          {rowFilters.map((filter) => (
            <MenuItem key={filter.id}>
              <FormControlLabel
                ...
                label={filter.label}
                onChange={() => handleToggleFilter(filter.id, !filter.state)}
              />
            </MenuItem>
          ))}
        </Menu>
