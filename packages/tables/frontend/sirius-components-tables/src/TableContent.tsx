...
const isCheckboxCell = (cell: GQLCell): cell is GQLCheckboxCell => cell.cellType === 'CheckboxCell';
const isSelectCell = (cell: GQLCell): cell is GQLSelectCell => cell.cellType === 'SelectCell';
const isMultiSelectCell = (cell: GQLCell): cell is GQLMultiSelectCell => cell.cellType === 'MultiSelectCell';
const isTextfieldCell = (cell: GQLCell): cell is GQLTextfieldCell => cell.cellType === 'TextfieldCell';
...
export const TableContent = ({ editingContextId, representationId, table, readOnly }: TableProps) => {
  ...
  const getCellLabel = (cell: GQLCell): string => {
    let value = '';
  ...
  const columns = useMemo<MRT_ColumnDef<GQLLine, string>[]>(() => {
    let columnDefs: MRT_ColumnDef<GQLLine, string>[] = table.columns.map((col) => {
      return {
        ...
        size: 150,
        Cell: ({ row }) => {
          const gqlCell: GQLCell = row.original.cells.find((cell) => col.id === cell.columnId);
          if (isCheckboxCell(gqlCell)) {
        ...
        muiEditTextFieldProps: (props: { cell: MRT_Cell<GQLLine, string>; row: MRT_Row<GQLLine> }): TextFieldProps => {
          ...
          const gqlCell: GQLCell = line.cells.find((cell) => col.id === cell.columnId);
