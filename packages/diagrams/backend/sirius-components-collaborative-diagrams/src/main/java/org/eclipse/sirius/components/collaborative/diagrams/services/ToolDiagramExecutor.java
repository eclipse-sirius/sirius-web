 ...
 */
@Service
public class ToolDiagramExecutor implements IToolRepresentationExecutor {
    ...
    public ToolDiagramExecutor(IToolService toolService, IObjectService objectService, IDiagramQueryService diagramQueryService) {
    ...
    @Override
    public boolean canExecute(IEditingContext editingContext, IRepresentation representation) {
        return representation instanceof Diagram;
    }

    @Override
    public IStatus execute(IEditingContext editingContext, IRepresentation representation, String toolId, String targetObjectId, List<ToolVariable> variables) {
        if (representation instanceof Diagram diagram) {
            ...
            if (optionalTool.isPresent()) {
                return this.executeTool(editingContext, diagramContext, targetObjectId, optionalTool.get(), variables);
            }

        }

        return new Failure(String.format("The tool %s cannot be found on the current diagram %s and editing context %s", toolId, representation.getId(), editingContext.getId()));
    }
