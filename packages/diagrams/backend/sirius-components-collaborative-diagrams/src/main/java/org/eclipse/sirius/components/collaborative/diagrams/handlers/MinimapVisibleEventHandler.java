...
public class MinimapVisibleEventHandler implements IDiagramEventHandler {
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, DiagramContext diagramContext, IDiagramInput diagramInput) {
        ...
        //By default, to keep the same behavior, if the minimap visibility can't be computed, we return a "true" value.
        IPayload payload = new MinimapVisiblePayload(diagramInput.id(), true);
        ...
        if (diagramInput instanceof MinimapVisibleInput) {
            ...
            if (optionalDiagramDescription.isPresent()) {
                ...
                boolean minimapVisible = this.minimapVisibilityProviders.stream()
                        .filter(provider -> provider.canHandle(editingContext, diagramDescription))
                        .allMatch(provider -> provider.isMinimapVisible(editingContext, diagramDescription, targetObjectId));

                payload = new MinimapVisiblePayload(diagramInput.id(), minimapVisible);
            }
        }
