...
public class InvokeActionEventHandler implements IDiagramEventHandler {
    ...
    public InvokeActionEventHandler(IObjectSearchService objectSearchService, IRepresentationDescriptionSearchService representationDescriptionSearchService, IDiagramDescriptionService diagramDescriptionService, IDiagramQueryService diagramQueryService,
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, IDiagramContext diagramContext, IDiagramInput diagramInput) {
        ...
        if (diagramInput instanceof InvokeActionInput input) {
            ...
            var optionalNodeDescription = this.findNodeDescription(diagram, input.targetElementId(), optionalDiagramDescription.get(), optionalNode);
            var optionalAction = this.getAction(actionId, optionalNodeDescription, optionalNode, editingContext, diagramContext, input.diagramElementId());
    ...
    private Optional<Action> getAction(String actionId, Optional<NodeDescription> optionalNodeDescription, Optional<Node> node, IEditingContext editingContext, IDiagramContext diagramContext, String diagramElementId) {
        Optional<Action> optionalAction = Optional.empty();
        if (optionalNodeDescription.isPresent()) {
            NodeDescription nodeDescription = optionalNodeDescription.get();
            var self = this.getCurrentContext(editingContext, diagramElementId, node);
            if (self.isPresent()) {
                var variableManager = this.populateVariableManager(editingContext, diagramContext, node, self);
                optionalAction = nodeDescription.getActionsProvider()
                        .apply(variableManager)
                        .stream()
    ...
    private Optional<Object> getCurrentContext(IEditingContext editingContext, String diagramElementId, Optional<Node> node) {
        Optional<Object> self = Optional.empty();
        if (node.isPresent()) {
            self = this.objectSearchService.getObject(editingContext, node.get().getTargetObjectId());
        } else {
            this.logger.warn("The action cannot be applied on the current node");
        }
        return self;
    }
