...
public class HideDiagramElementEventHandler implements IDiagramEventHandler {
    ...
    public HideDiagramElementEventHandler(ICollaborativeDiagramMessageService messageService, MeterRegistry meterRegistry, IDiagramQueryService diagramQueryService, IFeedbackMessageService feedbackMessageService) {
    ...
    private void handleHideDiagramElement(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, DiagramContext diagramContext, HideDiagramElementInput diagramInput) {
        ...
        if (resolvedIds.size() > 0) {
            diagramContext.diagramEvents().add(new HideDiagramElementEvent(resolvedIds, diagramInput.hide()));
        }
