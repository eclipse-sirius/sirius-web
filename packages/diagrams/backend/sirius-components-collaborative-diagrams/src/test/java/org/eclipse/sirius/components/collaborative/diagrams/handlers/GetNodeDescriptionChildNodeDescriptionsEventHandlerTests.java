...
import org.eclipse.sirius.components.collaborative.diagrams.dto.GetNodeDescriptionChildNodeDescriptionsInput;
...
/**
 * Test for {@link GetNodeDescriptionChildNodeDescriptionsEventHandler}
 *
 * @author frouene
 */
class GetNodeDescriptionChildNodeDescriptionsEventHandlerTests {
    ...
    public void testGetNodeDescriptionBorderNodeDescriptions() {
        ...
        var handler = new GetNodeDescriptionChildNodeDescriptionsEventHandler(representationDescriptionSearchService, new ICollaborativeDiagramMessageService.NoOp(),
                new SimpleMeterRegistry());

        var input = new GetNodeDescriptionChildNodeDescriptionsInput(UUID.randomUUID(), UUID.randomUUID().toString(), DIAGRAM_ID, nodeDescription);
        ...
        assertThat(nodeDescriptions.stream().map(NodeDescription::getId).toList()).contains(CHILD_NODE_DESCRIPTION_ID);
        assertThat(nodeDescriptions.stream().map(NodeDescription::getId).toList()).contains(REUSED_CHILD_NODE_DESCRIPTION_ID);
    }
