...
public class NodeComponent implements IComponent {
    ...
    private Element doRender(VariableManager nodeVariableManager, String targetObjectId, Optional<Node> optionalPreviousNode, List<IDiagramEvent> diagramEvents) {
        ...
        Set<String> customizedStyleProperties = optionalPreviousNode.map(Node::getCustomizedStyleProperties).orElse(new HashSet<>());
        ...
        Optional<INodeStyle> optionalPreviousNodeStyle = optionalPreviousNode.map(Node::getStyle);
        INodeStyle style = getNodeAppearanceHandlers(nodeVariableManager)
                .stream()
                .filter(handler -> handler.canHandle(providedStyle))
                .findFirst()
                .map(handler -> handler.handle(providedStyle,
                        appearanceChanges,
                        optionalPreviousNodeStyle,
                        customizedStyleProperties))
                .orElse(providedStyle);
