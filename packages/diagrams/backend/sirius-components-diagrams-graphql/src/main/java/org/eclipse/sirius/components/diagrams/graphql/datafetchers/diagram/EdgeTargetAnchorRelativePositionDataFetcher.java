 ...
 * @author sbegaudeau
 */
@QueryDataFetcher(type = "Edge", field = "targetAnchorRelativePosition")
public class EdgeTargetAnchorRelativePositionDataFetcher implements IDataFetcherWithFieldCoordinates<Ratio> {
    @Override
    public Ratio get(DataFetchingEnvironment environment) throws Exception {
        ...
        return Optional.ofNullable(localContext.get("diagram"))
                ...
                .flatMap(diagram -> this.targetAnchorRelativePosition(diagram, edge))
                .orElse(new Ratio(0, 0));
    }

    private Optional<Ratio> targetAnchorRelativePosition(Diagram diagram, Edge edge) {
        ...
        if (diagram.getLabel().endsWith("__EXPERIMENTAL")) {
            optionalRatio = Optional.of(diagram.getLayoutData())
                    ...
                    .map(EdgeLayoutData::targetAnchorRelativePosition);
        } else {
            optionalRatio = Optional.of(new Ratio(edge.getTargetAnchorRelativePosition().getX(), edge.getTargetAnchorRelativePosition().getY()));
        }
