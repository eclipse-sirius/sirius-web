...
public class ConnectorToolsIconURLsDataFetcher implements IDataFetcherWithFieldCoordinates<List<String>> {
    ...
    public ConnectorToolsIconURLsDataFetcher(IImageURLSanitizer imageURLSanitizer) {
        this.imageURLSanitizer = imageURLSanitizer;
    }

    @Override
    public List<String> get(DataFetchingEnvironment environment) throws Exception {
        ConnectorTool tool = environment.getSource();
        return tool.iconURL().stream().map(url -> this.imageURLSanitizer.sanitize(URLConstants.IMAGE_BASE_PATH, url)).toList();
    }
}
