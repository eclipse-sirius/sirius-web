...
const applyMultiSelectResize = (
  ...
  return nodes
    ...
    .map((node) => {
      let newWidth: number = change.dimensions?.width ?? node.width ?? 0;
      let newHeight: number = change.dimensions?.height ?? node.height ?? 0;
  ...
  const transformResizeListNodeChanges = useCallback(
    (changes: NodeChange<Node<NodeData>>[]): NodeChange<Node<NodeData>>[] => {
      ...
      const updatedChanges: NodeChange<Node<NodeData>>[] = changes.map((change) => {
        ...
        if (isResize(change)) {
          newMultiSelectResizeChanges.push(...applyMultiSelectResize(change, selection, getNodes()));
        }
        return change;
      });
      return [
