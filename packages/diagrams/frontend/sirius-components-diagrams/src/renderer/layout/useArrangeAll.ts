...
export const useArrangeAll = (reactFlowWrapper: React.MutableRefObject<HTMLDivElement | null>): UseArrangeAllValue => {
  ...
  const arrangeAllWithLayeredAlgorithm = async (): Promise<void> => {
    await arrangeAllWithOptions(elkLayeredOptions(diagramDescription.arrangeLayoutDirection));
  };

  const arrangeAllWithRectPackingAlgorithm = async (): Promise<void> => {
    await arrangeAllWithOptions(elkRectPackingOptions);
  };

  return {
    arrangeAllWithLayeredAlgorithm,
    arrangeAllWithRectPackingAlgorithm,
  };
};
