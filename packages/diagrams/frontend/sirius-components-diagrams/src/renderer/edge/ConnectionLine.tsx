...
const isEdgeAnchorNodeCreationHandles = (
  node: InternalNode<Node<NodeData>>
): node is InternalNode<Node<IconLabelNodeData>> => node.type === 'iconLabelNode';
...
export const ConnectionLine = memo(
  ({
    ...
    let nodeToSnap = toNode;
    if (nodeToSnap && isEdgeAnchorNodeCreationHandles(nodeToSnap)) {
      nodeToSnap = store.getState().nodeLookup.get(nodeToSnap.parentId || '') || null;
    }

    if (handleToUpdate && edgeId && nodeToSnap && nodeToSnap.width && nodeToSnap.height) {
      // Snap the ConnectionLine to the border of the targetted node
      const pointToSnap = getNearestPointInPerimeter(
        nodeToSnap.internals.positionAbsolute.x,
        nodeToSnap.internals.positionAbsolute.y,
        nodeToSnap.width,
        nodeToSnap.height,
        toX,
        toY
    ...
    useEffect(() => {
      if (nodeToSnap && edgeId && handleToUpdate) {
        ...
        setNodes(nodes);
        updateNodeInternals(nodeToSnap.id);
      }
    }, [toX, toY]);
