...
const equalityFunction = (oldMarkerProps: MarkerProps[], newMarkerProps: MarkerProps[]) => {
  return !(
    ...
    oldMarkerProps.some(
      (edge, index) =>
        !!newMarkerProps[index] &&
        (edge.id !== newMarkerProps[index]?.id || edge.edgeColor !== newMarkerProps[index]?.edgeColor)
    )
  );
};
...
export const MarkerDefinitions = () => {
  ...
  return (
    <svg id="edge-markers" style={{ position: 'absolute', zIndex: -1, top: '0px' }}>
      <defs data-svg="defs" id="edge-markers-defs">
        {allMarkerProps.map((markerProps) => getMarker(markerProps))}
      </defs>
    </svg>
  );
};
