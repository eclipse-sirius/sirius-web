...
export const getEdgeParameters: GetEdgeParameters = (source, target, nodeLookup, layoutDirection, bendingPoints) => {
  const firstBendingPoint = bendingPoints && bendingPoints.length > 0 ? bendingPoints[0] : null;
  const lastBendingPoint = bendingPoints && bendingPoints.length > 0 ? bendingPoints[bendingPoints.length - 1] : null;
