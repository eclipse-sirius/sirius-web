...
const buildSignature = (edges: Edge<MultiLabelEdgeData>[]): string => {
  return edges
    .map((edge) => `${edge.id}:${edge.data?.edgePath ?? ''}:${edge.source}->${edge.target}`)
    .join('|');
};
...
const areGapsEqual = (first?: EdgeCrossingGap[], second?: EdgeCrossingGap[]): boolean => {
  ...
  return safeFirst.every((gap, index) => {
    ...
    return (
      Math.abs(gap.startRatio - otherGap.startRatio) < 0.001 && Math.abs(gap.endRatio - otherGap.endRatio) < 0.001
    );
  });
};
...
export const useEdgeCrossingFades = (): void => {
  ...
  const { dragging } = useContext(DropNodeContext);
  ...
  useEffect(() => {
    ...
    const crossingWindows = detectEdgeCrossings(edges, nodes);
  ...
  }, [dragging, edges, setEdges, signature]);
};
