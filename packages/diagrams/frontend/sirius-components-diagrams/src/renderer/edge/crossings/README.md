...
2. **Hook** – `useEdgeCrossingFades()` pulls the current `edges` snapshot from
   the store, reuses React Flow’s `nodeLookup`, and calls `detectEdgeCrossings`.
...
```bash
cd packages/diagrams/frontend/sirius-components-diagrams
npx vitest --run --config vite.config.js src/renderer/edge/crossings/__tests__
```
