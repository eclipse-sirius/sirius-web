...
/**
 * Build a node lookup map keyed by id, mirroring the React Flow store shape the detector expects.
 */
const buildNodeLookup = (...nodes: TestNode[]): Map<string, TestNode> => {
...
describe('detectEdgeCrossings', () => {
  it('identifies orthogonal crossings and fades the higher z-index edge', () => {
    ...
    const windows = detectEdgeCrossings([verticalEdge, horizontalEdge], new Map());
  ...
  it('ignores intersections that lie near either path endpoint', () => {
    ...
    // Act.
    const windows = detectEdgeCrossings([shortVertical, horizontal], new Map());
  ...
  it('ignores intersections occurring within the padded bounds of connected nodes', () => {
    ...
    // Act.
    const nodeLookup = buildNodeLookup(sharedNode, targetA, targetB);
    const windows = detectEdgeCrossings([edgeA, edgeB], nodeLookup);
  ...
  it('falls back to deterministic edge ids when z-indexes are missing', () => {
    ...
    const windows = detectEdgeCrossings([first, second], new Map(), 10);
