...
import { EdgeCrossingMap, EdgeCrossingWindow } from './edgeCrossing.types';
...
type EdgeWithEndpoints = Edge<MultiLabelEdgeData> & {
...
const isPointNearEdgeEndpoints = (
  ...
  const edgeWithEndpoints = edge.edge as EdgeWithEndpoints;
  const { sourceNode, targetNode } = edgeWithEndpoints;
  const resolvedSource = sourceNode ?? nodeLookup.get(edge.edge.source);
  const resolvedTarget = targetNode ?? nodeLookup.get(edge.edge.target);
  const nodes: Array<Node<NodeData> | undefined> = [resolvedSource, resolvedTarget];
 ...
 * @param nodes - Current diagram nodes, used to avoid fading inside node bounds.
...
export const detectEdgeCrossings = (
  ...
  const nodeLookup = nodes.reduce<Map<string, Node<NodeData>>>((lookup, node) => {
  ...
  const parsedEdges: ParsedEdge[] = edges
    .map((edge) => {
      ...
      const withEndpoints = edge as EdgeWithEndpoints;
      if (!withEndpoints.sourceNode && nodeLookup.has(edge.source)) {
        withEndpoints.sourceNode = nodeLookup.get(edge.source);
      }
      if (!withEndpoints.targetNode && nodeLookup.has(edge.target)) {
        withEndpoints.targetNode = nodeLookup.get(edge.target);
      }
      return { edge: withEndpoints, segments, totalLength };
    })
