...
export const GroupPaletteLayoutPaletteEntry = ({ onToolSectionClick }: PaletteExtensionEntryComponentProps) => {
  const { classes } = useStyle();
  const { t } = useTranslation('sirius-components-diagrams', { keyPrefix: 'diagramPalette' });

  const title = t('layout');

  return (
    <Tooltip key={`tooltip_layout_section`} title={title} placement="right">
      <ListItemButton
        className={classes.listItemButton}
        onClick={(event) => onToolSectionClick(event, 'layout_section')}
