 ...
 *******************************************************************************/
import { useMousePosition, UseMousePositionValue, useMultiToast } from '@eclipse-sirius/sirius-components-core';
...
const getAllPaletteTools = (palette: GQLPalette) => {
  return palette.quickAccessTools
    ...
    .concat(palette.paletteEntries.filter(isToolSection).flatMap((section) => section.tools));
};

const computeCursorPosition = (
  mousePosition: UseMousePositionValue,
  viewport: Viewport,
...
const toolHasKeyBinding = (tool: GQLTool, event: React.KeyboardEvent<Element>) => {
...
export const useDiagramKeyBinding = (diagramTargetObjectId: string): UseDiagramKeyBindingValue => {
  ...
  const getSelectedDiagramElementIds = () => {
    return getNodes()
  ...
  const { getPaletteContents, palette, loading } = usePaletteContents();
  ...
  useEffect(() => {
    if (!loading && palette && state.currentEvent) {
      ...
      const selectedDiagramElementIds = getSelectedDiagramElementIds();
      const targetObjectId = getTargetObjectId(selectedDiagramElementIds);
      if (toolToExecute.length === 1 && toolToExecute[0]) {
        invokeSingleClickTool(
          editingContextId,
          diagramId,
          toolToExecute[0],
          selectedDiagramElementIds[0] ? selectedDiagramElementIds : [diagramId],
          targetObjectId,
          x,
          y
        );
  ...
  const onKeyBinding = useCallback(
    (event: React.KeyboardEvent<Element>) => {
      ...
      // We have to call preventDefault here and not in the useEffect (once we are sure there is a tool for the key binding):
      ...
      getPaletteContents({
        variables: {
          editingContextId,
          diagramId,
          diagramElementIds: selectedDiagramElementIds[0] ? selectedDiagramElementIds : [diagramId],
        },
      });
    },
    [diagramId]
