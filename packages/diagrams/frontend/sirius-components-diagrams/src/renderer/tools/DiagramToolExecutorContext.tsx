...
import {
  ...
  DiagramToolExecutorContextProviderState,
...
export const DiagramToolExecutorContextProvider = ({ children }: DiagramToolExecutorContextProviderProps) => {
  const [state, setState] = useState<DiagramToolExecutorContextProviderState>({
    x: 0,
    y: 0,
  ...
  const { invokeTool } = useInvokePaletteTool({
    ...
    onDirectEditClick: state.onDirectEditClick,
    targetObjectId: state.targetObjectId ?? '',
  });

  useEffect(() => {
    if (state.tool) {
      invokeTool(state.tool);
    }
  }, [state.x, state.y, state.diagramElementId, state.targetObjectId, state.onDirectEditClick, state.tool]);
