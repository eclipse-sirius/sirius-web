 ...
 *******************************************************************************/
import { WorkbenchViewConfigurationSupplier } from '@eclipse-sirius/sirius-components-core';
...
export const Form = forwardRef<WorkbenchViewConfigurationSupplier | null, FormProps>(
  (
    { editingContextId, form, readOnly, initiallySelectedPageId }: FormProps,
    workbenchViewConfigurationSupplierRef: ForwardedRef<WorkbenchViewConfigurationSupplier | null>
  ) => {
    const { classes } = useFormStyles();
    const { id, pages } = form;

    const initiallySelectedPage: GQLPage = initiallySelectedPageId
      ? pages.find((page) => page.id === initiallySelectedPageId) ?? pages[0]
      : pages[0];

    const [state, setState] = useState<FormState>({ selectedPage: initiallySelectedPage, pages });

    useImperativeHandle(
      workbenchViewConfigurationSupplierRef,
      () => {
        return {
