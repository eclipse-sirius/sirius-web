...
/**
 * Handle "Get non-working days" events.
 *
 * @author ncouvert
 */
@Service
public class GetNonWorkingDaysEventHandler implements IGanttEventHandler {

    private final IGanttNonWorkingDaysService ganttNonWorkingDaysService;

    private final ICollaborativeGanttMessageService messageService;

    private final Counter counter;

    public GetNonWorkingDaysEventHandler(IGanttNonWorkingDaysService ganttNonWorkingDaysService, ICollaborativeGanttMessageService messageService, MeterRegistry meterRegistry) {
        this.ganttNonWorkingDaysService = Objects.requireNonNull(ganttNonWorkingDaysService);
    ...
    @Override
    public boolean canHandle(IEditingContext editingContext, IGanttInput ganttInput) {
        return ganttInput instanceof GetNonWorkingDaysInput;
    }
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, IGanttContext ganttContext, IGanttInput ganttInput) {
        ...
        String message = this.messageService.invalidInput(ganttInput.getClass().getSimpleName(), GetNonWorkingDaysInput.class.getSimpleName());
        ...
        if (ganttInput instanceof GetNonWorkingDaysInput input) {
            payload =  this.ganttNonWorkingDaysService.getNonWorkingDays(input, editingContext, ganttContext.getGantt());
        }
