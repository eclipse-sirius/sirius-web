...
public class FlowTopographyGroupPaletteProvider {
    ...
    public GroupPalette createPalette(IViewDiagramElementFinder cache) {
        var optionalReusedToolSection = cache.getNodeDescription(SystemDescriptionProvider.NAME)
                .flatMap(nodeDescription -> nodeDescription.getPalette().getToolSections().stream().findFirst());

        var palette = this.diagramBuilderHelper.newGroupPalette()
                .toolSections(new DefaultToolsFactory().createDefaultHideRevealNodeToolSection());
        optionalReusedToolSection.ifPresent(palette::reusedToolSection);
