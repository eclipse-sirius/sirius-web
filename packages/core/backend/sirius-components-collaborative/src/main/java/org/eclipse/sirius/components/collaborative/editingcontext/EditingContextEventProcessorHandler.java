...
/**
 * Use to handle inputs from the event processors.
 *
 * @author mcharfadi
 */
@Service
public class EditingContextEventProcessorHandler implements IEditingContextEventProcessorHandler {

    private static final String LOG_TIMING_FORMAT = "%1$6s";

    private final Logger logger = LoggerFactory.getLogger(EditingContextEventProcessorHandler.class);
    ...
    @Override
    public void handle(ExecutorService executorService, Sinks.One<IPayload> payloadSink, Sinks.Many<Boolean> canBeDisposedSink, Sinks.Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, IInput input) {
    ...
    private void handleRepresentationInput(ExecutorService executorService, Sinks.One<IPayload> payloadSink, Sinks.Many<Boolean> canBeDisposedSink, Sinks.Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, IRepresentationInput representationInput) {
        Optional<IRepresentationEventProcessor> optionalRepresentationEventProcessor = this.representationEventProcessorGetter.acquireRepresentationEventProcessor(executorService, canBeDisposedSink, editingContext, representationInput.representationId(), representationInput);
