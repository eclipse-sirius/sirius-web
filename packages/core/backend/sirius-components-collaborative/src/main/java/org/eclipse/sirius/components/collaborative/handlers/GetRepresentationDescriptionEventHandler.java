...
public class GetRepresentationDescriptionEventHandler implements IEditingContextEventHandler {
    ...
    public GetRepresentationDescriptionEventHandler(IRepresentationDescriptionSearchService representationDescriptionSearchService) {
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, IInput input,
        ...
        if (input instanceof GetRepresentationDescriptionInput getRepresentationDescriptionInput) {
            // @formatter:off
            var optionalRepresentationDescriptionId = representationEventProcessors.stream()
                    .map(IRepresentationEventProcessor::getRepresentation)
                    .filter(representation -> representation.getId().equals(getRepresentationDescriptionInput.representationId()))
                    .map(IRepresentation::getDescriptionId)
                    .findFirst();
            // @formatter:on

            IRepresentationDescription representationDescription = null;
            if (optionalRepresentationDescriptionId.isPresent()) {
                var representationDescriptionId = optionalRepresentationDescriptionId.get();
                representationDescription = this.representationDescriptionSearchService.findById(editingContext, representationDescriptionId).orElse(null);
            }
