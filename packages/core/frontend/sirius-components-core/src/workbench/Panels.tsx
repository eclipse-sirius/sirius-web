...
import { PanelsProps, PanelState, WorkbenchPanelHandle, WorkbenchViewConfiguration } from './Panels.types';
...
export const Panels = forwardRef<WorkbenchPanelHandle, PanelsProps>(
  (
    {
      editingContextId,
      readOnly,
    ...
    ref: ForwardedRef<WorkbenchPanelHandle>
  ) => {
    disableGlobalCursorStyles();
    ...
    useImperativeHandle(
      ref,
      () => {
        const workbenchPanelHandle: WorkbenchPanelHandle = {
          getPanelConfigurations: () => {
            const leftPanelViews: WorkbenchViewConfiguration[] = leftContributions.map((contribution) => ({
              id: contribution.id,
              active: contribution.id === leftContribution?.id,
            }));
            const rightPanelViews: WorkbenchViewConfiguration[] = rightContributions.map((contribution) => ({
              id: contribution.id,
              active: contribution.id === rightContribution?.id,
            }));
            return [
              { id: 'left', views: leftPanelViews },
              { id: 'right', views: rightPanelViews },
            ];
          },
        };

        return workbenchPanelHandle;
      },
