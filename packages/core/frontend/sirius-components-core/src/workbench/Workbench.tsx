...
import { WorkbenchPanelConfiguration, WorkbenchPanelHandle } from './Panels.types';
...
export const Workbench = forwardRef<WorkbenchHandle, WorkbenchProps>(
  (
    { editingContextId, initialRepresentationSelected, onRepresentationSelected, readOnly }: WorkbenchProps,
    ref: ForwardedRef<WorkbenchHandle>
  ) => {
    ...
    const workbenchPanelsRef = useRef<WorkbenchPanelHandle | null>(null);

    useImperativeHandle(ref, () => {
      const workbenchHandle: WorkbenchHandle = {
        getConfiguration: () => {
          let panels: WorkbenchPanelConfiguration[] = [];
          if (workbenchPanelsRef.current) {
            panels = workbenchPanelsRef.current.getPanelConfigurations();
          }
          return { panels };
        },
      };
      return workbenchHandle;
    });
