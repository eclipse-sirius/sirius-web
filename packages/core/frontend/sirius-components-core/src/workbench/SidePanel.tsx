...
export const SidePanel = forwardRef<SidePanelHandle, SidePanelProps>(
  (
    ...
    const handleContributionClicked = (id: string) => {
      setPanelState((prevState) => {
        if (prevState.selectedContributionIds.includes(id)) {
          const newSelected = prevState.selectedContributionIds.filter((contributionId) => contributionId !== id);
          return {
            ...prevState,
            selectedContributionIds: newSelected,
            collapsedContributionIds: prevState.collapsedContributionIds.filter(
              (contributionId) => contributionId !== id
            ),
            isOpen: newSelected.length === 0 ? false : prevState.isOpen,
          };
        } else {
          return {
            ...prevState,
    ...
    useEffect(() => {
      ...
      if (panelState.isOpen && panelRef.current.isCollapsed()) {
        const targetSize = lastExpandedSizeRef.current ?? panelInitialSize;
        panelRef.current.resize(`${targetSize}%`);
      } else if (!panelState.isOpen && !panelRef.current.isCollapsed()) {
    ...
    return (
      <PanelCollapseContextProvider onCollapseChange={handleCollapseChange}>
        <Panel
          ...
          defaultSize={initialState.isOpen ? `${panelInitialSize}%` : `${collapsedSize}%`}
          collapsible
          ...
          onResize={(size, _id, prevSize) => {
            ...
            if (prevSize !== undefined) {
              const wasOpen = prevSize.asPercentage !== 0;
