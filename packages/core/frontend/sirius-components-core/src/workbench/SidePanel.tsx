...
export const SidePanel = forwardRef<WorkbenchPanelsHandle | null, SidePanelProps>(
  (
    ...
    const changeCollapsePanelState = () => {
      // A view's panel constraints (min/max) are applied by the library one frame after its collapsed state changes.
      // Setting the layout on the next frame ensures setLayout validates against the up-to-date constraints.
      // Otherwise, it clamps the freshly expanded panel back to its (collapsed) size.
      requestAnimationFrame(() => {
        ...
        // react-resizable-panels reads layout object values by enumeration order, so insert properties in the same order as the panels are rendered
