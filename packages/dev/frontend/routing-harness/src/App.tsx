...
import {
  DiagramViewer,
  type DiagramViewerHandle,
  type EdgeRoutingAlgorithm,
} from "./components/DiagramViewer";
...
const EDGE_ROUTING_ALGORITHMS: EdgeRoutingAlgorithm[] = [
  "experimental",
  "manhattan",
  "smartManhattan",
  "oblique",
...
const findEdgeRoutingAlgorithm = (
  value: string | null
  ...
  return EDGE_ROUTING_ALGORITHMS.find(
...
export default function App() {
  ...
  const edgeAlgorithmOverride = useMemo<
    EdgeRoutingAlgorithm | undefined
  >(() => {
    if (typeof window === "undefined") {
      return undefined;
  ...
  const exportRange = useMemo<{ start: number; count: number | null }>(() => {
    ...
    const url = new URL(window.location.href);
    const startParam = Number.parseInt(
      url.searchParams.get("batchStart") ??
        url.searchParams.get("exportStart") ??
        "",
      10
    );
    const countParam = Number.parseInt(
      url.searchParams.get("batchCount") ??
        url.searchParams.get("exportCount") ??
        "",
      10
    );

    const start =
      Number.isFinite(startParam) && startParam > 0
        ? Math.max(0, startParam)
        : 0;
    const count =
      Number.isFinite(countParam) && countParam > 0
        ? Math.max(1, countParam)
        : null;
  ...
  const fixturesToDisplay = useMemo<LoadedFixture[]>(() => {
    ...
    const endIndex =
      exportRange.count !== null
        ? Math.min(startIndex + exportRange.count, fixtures.length)
        : fixtures.length;
  ...
  const rangeEndIndex =
    fixturesToDisplay.length > 0
      ? rangeStartIndex + fixturesToDisplay.length
      : rangeStartIndex;
  const rangeSummary =
    hasExportRange && totalFixtures > 0
      ? fixturesToDisplay.length > 0
        ? `Showing fixtures ${
            rangeStartIndex + 1
          }\u2013${rangeEndIndex} of ${totalFixtures}`
  ...
  const registerViewerHandle = useCallback(
    (fixtureId: string, handle: DiagramViewerHandle | null) => {
      const handles = viewerHandlesRef.current;
      if (handle) {
  ...
  const handleExportAll = useCallback(async () => {
    ...
    try {
      ...
      setExportFeedback({
        type: "success",
        message: `Exported ${exportedCount} diagram${
          exportedCount === 1 ? "" : "s"
        }.`,
      });
    } catch (err) {
    ...
    <div className="app">
      <div className="app__actions">
        ...
        >
          {isExporting
            ? "Exporting diagrams…"
            : `Export diagrams (${fixturesToDisplay.length})`}
        </button>
        {rangeSummary ? (
          <span className="app__range-summary">{rangeSummary}</span>
        ) : null}
      ...
      {!error && isLoading ? (
        <div className="app__status">Loading fixtures…</div>
      ) : null}
