...
public class SelectionDescriptionDialogDataFetcher implements IDataFetcherWithFieldCoordinates<CompletableFuture<SelectionDescriptionDialog>> {
    ...
    @Override
    public CompletableFuture<SelectionDescriptionDialog> get(DataFetchingEnvironment environment) throws Exception {
        ...
        var input = new GetSelectionDescriptionDialogInput(UUID.randomUUID(), variables, selectionDescription);
        return this.editingContextDispatcher.dispatchQuery(editingContextId.get(), input)
                .filter(GetSelectionDescriptionDialogPayload.class::isInstance)
                .map(GetSelectionDescriptionDialogPayload.class::cast)
                .map(GetSelectionDescriptionDialogPayload::dialog)
                .toFuture();
    }
}
