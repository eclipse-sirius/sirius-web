...
public class ViewRowFilterProvider implements IRowFilterProvider {
    ...
    public ViewRowFilterProvider(IEditingContextSearchService editingContextSearchService, ViewRepresentationDescriptionPredicate viewRepresentationDescriptionPredicate,
            IViewRepresentationDescriptionSearchService viewRepresentationDescriptionSearchService,
            IViewAQLInterpreterFactory aqlInterpreterFactory, IObjectSearchService objectSearchService) {
            ...
            if (optionalTableDescription.isPresent()) {
                ...
                if (viewTableDescription.eContainer() instanceof View view) {
                    ...
                    result = viewTableDescription.getRowFilters().stream()
                            .map(vrfd -> this.convertRowFilterDescription(vrfd, interpreter, variableManager))
                            .toList();
                }
            }
        }
