...
public class ActionExecutor implements IActionExecutor {
    ...
    public ActionExecutor(IIdentityService identityService, IOperationExecutor operationExecutor, IFeedbackMessageService feedbackMessageService) {
    ...
    public IStatus executeAction(Action action, AQLInterpreter interpreter, VariableManager variableManager) {
        ...
        if (result.status() == OperationExecutionStatus.FAILURE) {
            ...
            errorMessages.add(new Message(String.format("Something went wrong while executing the action '%s'", action.getName()), MessageLevel.ERROR));
