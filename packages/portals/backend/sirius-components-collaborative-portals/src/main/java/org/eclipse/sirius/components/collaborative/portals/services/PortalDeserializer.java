...
/**
 * Used to deserialize a portal.
 *
 * @author pcdavid
 */
@Service
public class PortalDeserializer implements IRepresentationDeserializer {

    private final Logger logger = LoggerFactory.getLogger(PortalDeserializer.class);

    @Override
    public boolean canHandle(ObjectNode root) {
        return Optional.ofNullable(root.get("kind"))
                .map(JsonNode::asText)
                .filter(Portal.KIND::equals)
                .isPresent();
    }
    ...
    public Optional<IRepresentation> handle(JsonParser jsonParser, DeserializationContext context, ObjectNode root) {
        try {
            return Optional.of(context.readTreeAsValue(root, Portal.class));
