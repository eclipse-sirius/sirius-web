...
import { TreeItemClickResult, UseTreeSelectionValue } from './useTreeSelection.types';
    ...
    (
      ...
      for (const element of subtree.children) {
        ...
        if (element.hasChildren && element.expanded) {
          isSelecting = computeSelectedElementsInRange(
            element,
          ...
          if (accumulator.includes(firstSelectionItemId) && accumulator.includes(secondSelectionItemId)) {
            return isSelecting;
          }
        }
      }
    ...
    (
      ...
      if (allowMultiSelection && event.shiftKey) {
        ...
        if (currentPivotId) {
          ...
          computeSelectedElementsInRange(tree, currentPivotId, item.id, rangeSelection);
