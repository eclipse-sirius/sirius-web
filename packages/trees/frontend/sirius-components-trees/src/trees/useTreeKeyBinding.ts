...
export const useTreeKeyBinding = (editingContextId: string, treeId: string): UseTreeKeyBindingValue => {
  ...
  const skipFetchContextMenuEntries = !treeItemId || !state.currentEvent;
  ...
  useEffect(() => {
    if (!loading && contextMenuEntries && state.currentEvent && treeItemId) {
      ...
      if (entryToExecute.length === 1 && entryToExecute[0]) {
        invokeContextMenuEntry(editingContextId, treeId, treeItemId, entryToExecute[0], () => {});
  ...
  }, [contextMenuEntries, loading, state.currentEvent, treeItemId]);
