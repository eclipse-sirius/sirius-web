...
export const SET_CHECK_LIST_ITEM_CHECKED_COMMAND: LexicalCommand<boolean> = createCommand(
...
export function ListPlugin(): null {
  ...
  useEffect(() => {
    return mergeRegister(
      registerMarkdownChecklistTransformer(editor),
      editor.registerCommand(
        ...
        (event) => {
          ...
          if (!$isListNode(listNode) || listNode.getListType() !== 'bullet' || listNode.getChildrenSize() !== 1) {
            return false;
          }
      ...
      editor.registerCommand(
        ...
        (checked) => {
          ...
          const listItems = selection.getNodes().map(getListItem).filter($isListItemNode);
          if ($isListItemNode(selectedListItem) && !listItems.includes(selectedListItem)) {
