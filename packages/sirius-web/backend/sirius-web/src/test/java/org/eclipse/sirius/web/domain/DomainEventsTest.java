...
public class DomainEventsTest extends AbstractIntegrationTests {
    ...
    public void givenDocumentWhenNameModifiedDomainEventPublished() {
        ...
        this.semanticDataUpdateService.updateDocuments(projectId, Set.of(updatedDocument), Set.of(), semanticData.getDomains().stream().map(SemanticDataDomain::uri).collect(Collectors.toSet()));
        TestTransaction.flagForCommit();
        TestTransaction.end();
