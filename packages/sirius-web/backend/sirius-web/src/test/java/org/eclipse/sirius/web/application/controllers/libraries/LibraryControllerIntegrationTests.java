...
public class LibraryControllerIntegrationTests extends AbstractIntegrationTests {
    ...
    public void givenProjectWithDependenciesWhenDependencyIsUpdatedThenEditingContextContainsTheNewVersionOfDependency() {
        Optional<Library> optLibrary = this.librarySearchService.findByNamespaceAndNameAndVersion("papaya", "sirius-web-tests-data", "2.0.0");
        assertThat(optLibrary).isPresent();
        ...
        var input = new UpdateLibraryInput(UUID.randomUUID(), PapayaIdentifiers.PAPAYA_EDITING_CONTEXT_ID.toString(), optLibrary.get().getId().toString());
