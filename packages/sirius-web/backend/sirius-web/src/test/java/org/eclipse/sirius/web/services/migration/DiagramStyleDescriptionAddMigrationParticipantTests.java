...
/**
 * Integration tests of DiagramStyleDescriptionAddMigrationParticipant.
 *
 * @author frouene
 */
@Transactional
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
public class DiagramStyleDescriptionAddMigrationParticipantTests extends AbstractIntegrationTests {
    ...
    @Test
    @DisplayName("Given a project with an old model, DiagramStyleDescriptionAddMigrationParticipant migrates the model correctly")
    ...
    public void givenAnOldModelMigrationParticipantCanBeContributedToUpdateTheModel() {
        var optionalEditingContext = this.editingContextSearchService.findById(MigrationIdentifiers.MIGRATION_DIAGRAM_STYLE_DESCRIPTION.toString());
        assertThat(optionalEditingContext).isPresent();
        this.testIsMigrationSuccessful(optionalEditingContext.get());
    }

    @Test
    @DisplayName("Given an uploaded project with an old model, DiagramStyleDescriptionAddMigrationParticipant migrates the model correctly")
    @Sql(scripts = { "/scripts/migration.sql" }, executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
    @Sql(scripts = { "/scripts/cleanup.sql" }, executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD, config = @SqlConfig(transactionMode = SqlConfig.TransactionMode.ISOLATED))
    public void givenAnOldViewDiagramMigrationServiceIsExecutedProperly() {
        var content = """
                        ...
                        "descriptions": [
                          {
                            ...
                            "data": {
                              "name": "DiagramStyleDescription#migration_upload",
                ...
                """;
        this.uploadDocument(MigrationIdentifiers.MIGRATION_DIAGRAM_STYLE_DESCRIPTION.toString(), content);
    }
        ...
        if (editingContext instanceof EditingContext siriusWebEditingContext) {
            ...
            optionalDiagramDescription.ifPresent(representationDescription -> {
                if (representationDescription instanceof DiagramDescription diagramDescription) {
                    assertThat(diagramDescription.getStyle()).isNotNull();
                }
            });
        }
    }
