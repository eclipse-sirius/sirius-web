...
public class UndoEditImageNodeAppearanceControllerTests extends AbstractIntegrationTests {
    ...
    public void givenDiagramWithAnAlreadyCustomisedAppearanceWhenWeDeleteTheNodeAndUndoTheChangeThenCustomizedPropertyIsKept() {
        ...
        Runnable setInitialNodeCustomAppearance = () -> {
            var appearanceInput = new ImageNodeAppearanceInput("blue", 10, 10, LineStyle.Dash_Dot);
            var input = new EditImageNodeAppearanceInput(
                    UUID.randomUUID(),
                    PapayaIdentifiers.PAPAYA_EDITING_CONTEXT_ID.toString(),
                    diagramId.get(),
        ...
        Consumer<Object> updatedAfterDeleteDiagramElementConsumer = assertRefreshedDiagramThat(diagram -> {
            assertThat(diagram.getNodes())
                    .allMatch(node -> node.getOutsideLabels() == null || !node.getOutsideLabels().get(0).text().equals("sirius-web-application"));
        });
        ...
        Consumer<Object> updatedAfterCustomAppearanceDiagramContentConsumer = assertRefreshedDiagramThat(diagram -> {
            assertThat(diagram.getNodes())
                    ...
                    .allMatch(imageNodeStyle -> LineStyle.Dash_Dot.equals(imageNodeStyle.getBorderStyle()))
                    .allMatch(imageNodeStyle -> imageNodeStyle.getBorderRadius() == 10)
                    .allMatch(imageNodeStyle -> imageNodeStyle.getBorderSize() == 10);

            assertThat(diagram.getNodes())
                    .filteredOn(node -> node.getOutsideLabels() != null && !node.getOutsideLabels().isEmpty() && node.getOutsideLabels().get(0).text().equals("sirius-web-application"))
                    .allMatch(node -> node.getCustomizedStyleProperties()
                            .containsAll(List.of(ImageNodeAppearanceHandler.BORDER_RADIUS, EllipseNodeAppearanceHandler.BORDER_SIZE, EllipseNodeAppearanceHandler.BORDER_STYLE, EllipseNodeAppearanceHandler.BORDER_COLOR)));
        });
