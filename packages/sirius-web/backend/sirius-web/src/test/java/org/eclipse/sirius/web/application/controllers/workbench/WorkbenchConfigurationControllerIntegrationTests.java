...
public class WorkbenchConfigurationControllerIntegrationTests extends AbstractIntegrationTests {
    ...
    public void givenProjectWhenWorkbenchConfigurationIsRequestedThenItReturnsTheDefaultConfiguration() {
        ...
        List<Object> representationEditors = JsonPath.read(result, "$.data.viewer.editingContext.workbenchConfiguration.mainPanel.representationEditors");
        List<Object> workbenchPanels = JsonPath.read(result, "$.data.viewer.editingContext.workbenchConfiguration.workbenchPanels");
        assertThat(representationEditors).isEmpty();
        assertThat(workbenchPanels).hasSize(2);

        List<Object> leftPanelViews = JsonPath.read(result, "$.data.viewer.editingContext.workbenchConfiguration.workbenchPanels[?(@['id']=='left')].views[*]");
        List<String> leftPanelViewsIDs = JsonPath.read(result, "$.data.viewer.editingContext.workbenchConfiguration.workbenchPanels[?(@['id']=='left')].views[*].id");
        ...
        List<Object> rightPanelViews = JsonPath.read(result, "$.data.viewer.editingContext.workbenchConfiguration.workbenchPanels[?(@['id']=='right')].views[*]");
        List<String> rightPanelViewsIDs = JsonPath.read(result, "$.data.viewer.editingContext.workbenchConfiguration.workbenchPanels[?(@['id']=='right')].views[*].id");
