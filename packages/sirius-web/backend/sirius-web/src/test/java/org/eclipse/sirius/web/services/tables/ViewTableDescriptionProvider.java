    ...
    private TableDescription createTableDescription() {
        ...
        var rowDescription = new TableBuilders().newRowDescription()
                .semanticCandidatesExpression(
                        "aql:self.eAllContents()->filter({papaya::Type | papaya::Operation | papaya::Parameter})->select(t | not (activateRowFilterIds->includes('hide-failure') and (t.name = 'Failure')))->toPaginatedData(cursor,direction,size)")
