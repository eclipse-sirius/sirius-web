        ...
        if (editingContext instanceof EditingContext siriusWebEditingContext) {
            ...
            optionalFormDescription.ifPresent(representationDescription -> {
                if (representationDescription instanceof FormDescription formDescription) {
                    assertThat(formDescription.getPages().get(0).getGroups().get(0).getChildren().get(0))
                            .asInstanceOf(type(TextfieldDescription.class))
                            .returns("column", textfield -> textfield.getStyle().getFlexDirection());

                    assertThat(formDescription.getPages().get(0).getGroups().get(0).getChildren().get(2))
                            .asInstanceOf(type(CheckboxDescription.class))
                            .returns("row-reverse", textfield -> textfield.getStyle().getFlexDirection())
                            .returns("1 1 auto", textfield -> textfield.getStyle().getLabelFlex());

                    assertThat(formDescription.getPages().get(0).getGroups().get(0).getChildren().get(3))
                            .asInstanceOf(type(DateTimeDescription.class))
                            .returns("column", textfield -> textfield.getStyle().getFlexDirection());
                }
            });
        }
