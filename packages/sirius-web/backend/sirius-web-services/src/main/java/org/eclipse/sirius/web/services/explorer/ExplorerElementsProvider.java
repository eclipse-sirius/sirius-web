...
public class ExplorerElementsProvider implements IExplorerElementsProvider {
    ...
    private final List<IExplorerTreeAlteredContentProvider> filteredTreeContentProviders;

    public ExplorerElementsProvider(List<IExplorerTreeAlteredContentProvider> filteredTreeContentProviders) {
        this.filteredTreeContentProviders = Objects.requireNonNull(filteredTreeContentProviders);
    }
    ...
    public List<Object> getElements(VariableManager variableManager) {
        ...
        if (optionalEditingContext.isPresent()) {
            ...
            List<IExplorerTreeAlteredContentProvider> alteredContentProviders = this.filteredTreeContentProviders.stream()
                .filter(provider -> provider.canHandle(optionalEditingContext.get(), activeFilterIds))
                .toList();
            for (IExplorerTreeAlteredContentProvider provider : alteredContentProviders) {
