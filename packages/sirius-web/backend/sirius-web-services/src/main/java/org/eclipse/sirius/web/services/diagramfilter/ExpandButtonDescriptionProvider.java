...
/**
 * Provides the description of the expand button for the diagram filter's split button.
 *
 * @author gdaniel
 */
@Service
public class ExpandButtonDescriptionProvider implements IDiagramFilterActionContributionProvider {
    ...
    public ExpandButtonDescriptionProvider(IObjectService objectService, IDiagramFilterHelper diagramFilterHelper, IServicesMessageService messageService) {
    ...
    public ButtonDescription getButtonDescription() {
        return ButtonDescription.newButtonDescription("diagram-filter/split-button/expand")
                ...
                .labelProvider(variableManager -> "Expand")
                .iconURLProvider(variableManager -> List.of())
                .isReadOnlyProvider(variableManager -> false)
                .buttonLabelProvider(variableManager -> "Expand")
                .imageURLProvider(variableManager -> DiagramImageConstants.EXPAND_SVG)
                .pushButtonHandler(variableManager -> {
                    ...
                    boolean hasFailure = nodeIds.stream()
                        .map(nodeId -> this.diagramFilterHelper.sendDiagramEvent(variableManager, new UpdateCollapsingStateInput(UUID.randomUUID(), editingContext.getId(), diagram.getId(), nodeId, CollapsingState.EXPANDED)))
                ...
                .helpTextProvider(variableManager -> this.messageService.expandSelectedElements())
                .build();
    }
