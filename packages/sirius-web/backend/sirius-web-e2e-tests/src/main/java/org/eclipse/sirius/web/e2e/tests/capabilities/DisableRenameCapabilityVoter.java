...
@Profile("test")
@Service
public class DisableRenameCapabilityVoter implements ICapabilityVoter {

    private final IProjectSearchApplicationService projectSearchApplicationService;

    public DisableRenameCapabilityVoter(IProjectSearchApplicationService projectSearchApplicationService) {
    ...
    public CapabilityVote vote(String type, String identifier, String capability) {
        if (SiriusWebCapabilities.PROJECT.equals(type) && SiriusWebCapabilities.Project.RENAME.equals(capability) && identifier != null && !identifier.isBlank()) {
            var optionalProject = this.projectSearchApplicationService.findById(identifier);
            if (optionalProject.isPresent()) {
                var project = optionalProject.get();
                if (project.name().contains("Cypress - Disabled Rename Project")) {
