...
/**
 * Disable the download capability for a specific project.
 *
 * @author gcoutable
 */
@Profile("test")
@Service
public class DisableDownloadCapabilityVoter implements ICapabilityVoter {

    private final IProjectSearchApplicationService projectSearchApplicationService;

    public DisableDownloadCapabilityVoter(IProjectSearchApplicationService projectSearchApplicationService) {
    ...
    public CapabilityVote vote(String type, String identifier, String capability) {
        if (SiriusWebCapabilities.PROJECT.equals(type) && SiriusWebCapabilities.Project.DOWNLOAD.equals(capability) && identifier != null && !identifier.isBlank()) {
            var optionalProject = this.projectSearchApplicationService.findById(identifier);
            if (optionalProject.isPresent()) {
                var project = optionalProject.get();
                if (project.name().contains("Cypress - Disabled Download Project")) {
