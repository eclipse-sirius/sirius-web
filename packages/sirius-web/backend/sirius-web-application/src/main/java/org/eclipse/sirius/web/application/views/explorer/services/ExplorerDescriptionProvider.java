...
public class ExplorerDescriptionProvider implements IEditingContextRepresentationDescriptionProvider {
    ...
    private Object getTreeItemObject(VariableManager variableManager) {
        Object result = null;
        Optional<String> optTreeItemId = variableManager.get(TreeDescription.ID, String.class);
        var optEditingContext = variableManager.get(IEditingContext.EDITING_CONTEXT, IEditingContext.class);

        if (optEditingContext.isPresent() && optTreeItemId.isPresent()) {
            String treeItemId = optTreeItemId.get();
            if (treeItemId.startsWith(SETTING)) {
                ...
                var optObject = this.objectService.getObject(optEditingContext.get(), treeItemId);
                if (optObject.isPresent()) {
                    result = optObject.get();
                } else {
                    // this is a resource
                    var optEditingDomain = Optional.of(optEditingContext.get())
                            .filter(IEMFEditingContext.class::isInstance)
                            .map(IEMFEditingContext.class::cast)
                            .map(IEMFEditingContext::getDomain);

                    if (optEditingDomain.isPresent()) {
                        var editingDomain = optEditingDomain.get();
