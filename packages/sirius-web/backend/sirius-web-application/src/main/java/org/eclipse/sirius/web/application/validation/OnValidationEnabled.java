 ...
 *******************************************************************************/
package org.eclipse.sirius.web.application.validation;
...
/**
 * Custom condition used to figure out if support for the default validation is enabled or not.
 *
 * @author sbegaudeau
 */
public class OnValidationEnabled extends SpringBootCondition {
    @Override
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        } else if (siriusWebEnabledFeatures.contains(SiriusWebProperties.VALIDATION)) {
            message = ConditionMessage.forCondition(this.getClass().getSimpleName()).available("sirius.web.enabled=" + SiriusWebProperties.VALIDATION);
            outcome = ConditionOutcome.match(message);
        }

        if (siriusWebDisabledFeatures.contains(SiriusWebProperties.VALIDATION)) {
            message = ConditionMessage.forCondition(this.getClass().getSimpleName()).available("sirius.web.disabled=" + SiriusWebProperties.VALIDATION);
