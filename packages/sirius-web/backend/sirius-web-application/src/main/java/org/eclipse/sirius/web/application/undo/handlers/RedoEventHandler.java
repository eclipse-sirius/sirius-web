...
public class RedoEventHandler implements IEditingContextEventHandler {
    ...
    public RedoEventHandler(IRepresentationSearchService representationSearchService, ICollaborativeDiagramMessageService messageService) {
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, IInput input) {
        ...
        this.payload = new ErrorPayload(input.id(), message);
            ...
            if (siriusEditingContext.getRepresentationChangesDescription().get(redoInput.mutationId()) != null) {
                ...
                for (IRepresentationChangeEvent change : changes) {
                    if (change instanceof LayoutPortalRepresentionChange layoutPortalRepresentionChange) {
                        changeDescription = this.handleLayoutPortalChange(editingContext, redoInput, layoutPortalRepresentionChange);
                    }
                    if (change instanceof AddPortalRepresentionChange addPortalRepresentionChange) {
                        changeDescription = this.handleAddPortalChange(editingContext, redoInput, addPortalRepresentionChange);
                    }
                    if (change instanceof RemovePortalRepresentionChange removePortalRepresentionChange) {
                        changeDescription = this.handleRemovePortalChange(editingContext, redoInput, removePortalRepresentionChange);
                    }
                }
            }
        }
        payloadSink.tryEmitValue(this.payload);
