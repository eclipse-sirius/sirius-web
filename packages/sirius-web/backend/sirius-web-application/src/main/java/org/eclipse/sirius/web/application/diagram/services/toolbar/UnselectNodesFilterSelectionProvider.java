...
/**
 * Handler used to get the unselect nodes menu item of the FilterSelectionMenu in the diagram toolbar.
 *
 * @author mcharfadi
 */
@Service
public class UnselectNodesFilterSelectionProvider implements IDiagramToolbarFilterSelectionProvider {

    public static final String ID = "unselect_all_nodes";
    ...
    public boolean canHandle(IEditingContext editingContext, DiagramContext diagramContext, List<String> diagramElementIds) {
        return !diagramElementIds.isEmpty() && diagramElementIds.stream().anyMatch(diagramElementId -> this.diagramQueryService.findNodeById(diagramContext.diagram(), diagramElementId).isPresent());
    }
