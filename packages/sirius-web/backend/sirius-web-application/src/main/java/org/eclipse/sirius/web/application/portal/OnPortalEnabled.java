 ...
 *******************************************************************************/
package org.eclipse.sirius.web.application.portal;
...
/**
 * Custom condition used to figure out if support for portals is enabled or not.
 *
 * @author sbegaudeau
 */
public class OnPortalEnabled extends SpringBootCondition {
    @Override
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        } else if (siriusWebEnabledFeatures.contains(SiriusWebProperties.PORTAL)) {
            message = ConditionMessage.forCondition(this.getClass().getSimpleName()).available("sirius.web.enabled=" + SiriusWebProperties.PORTAL);
            outcome = ConditionOutcome.match(message);
        }

        if (siriusWebDisabledFeatures.contains(SiriusWebProperties.PORTAL)) {
            message = ConditionMessage.forCondition(this.getClass().getSimpleName()).available("sirius.web.disabled=" + SiriusWebProperties.PORTAL);
