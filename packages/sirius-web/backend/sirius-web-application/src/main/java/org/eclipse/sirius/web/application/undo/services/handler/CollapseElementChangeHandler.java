...
/**
 * Used to handle the undo/redo for the UpdateCollapsingStateEvent.
 *
 * @author mcharfadi
 */
@Service
public class CollapseElementChangeHandler implements IRepresentationChangeHandler {

    private final IRepresentationEventProcessorRegistry representationEventProcessorRegistry;

    public CollapseElementChangeHandler(IRepresentationEventProcessorRegistry representationEventProcessorRegistry) {
    ...
    public boolean canHandle(UUID inputId, IEditingContext editingContext) {
        return editingContext instanceof EditingContext siriusEditingContext && siriusEditingContext.getInputId2RepresentationChanges().get(inputId) != null &&
                siriusEditingContext.getInputId2RepresentationChanges().get(inputId).stream().anyMatch(DiagramCollapseElementChange.class::isInstance);
    }
    ...
    public void redo(UUID inputId, IEditingContext editingContext) {
        if (editingContext instanceof EditingContext siriusEditingContext) {
            siriusEditingContext.getInputId2RepresentationChanges().get(inputId).stream().filter(DiagramCollapseElementChange.class::isInstance)
                .map(DiagramCollapseElementChange.class::cast)
                .forEach(change -> {
                    ...
                    if (representationEventProcessorEntry != null && representationEventProcessorEntry.getRepresentationEventProcessor() instanceof DiagramEventProcessor eventProcessor) {
                        ...
                        diagramContext.diagramEvents().add(new UpdateCollapsingStateEvent(change.nodeId(), change.redoValue()));
                    }
                });
        }
    }
        ...
        if (editingContext instanceof EditingContext siriusEditingContext) {
            siriusEditingContext.getInputId2RepresentationChanges().get(inputId).stream().filter(DiagramCollapseElementChange.class::isInstance)
                ...
                .forEach(change -> {
                    ...
                    if (representationEventProcessorEntry != null && representationEventProcessorEntry.getRepresentationEventProcessor() instanceof DiagramEventProcessor eventProcessor) {
                        var diagramContext = eventProcessor.getDiagramContext();
                        diagramContext.diagramEvents().add(new UpdateCollapsingStateEvent(change.nodeId(), change.undoValue()));
                    }
                });
        }
    }
