            ...
            if (optionalContainer.isPresent()) {
                ...
                if (optAdapterFactoryEditingDomain.isPresent()) {
                    ...
                    if (adapter instanceof IEditingDomainItemProvider editingDomainItemProvider && editingDomainItemProvider instanceof Helper helper) {
                        ...
                        List<CommandParameter> commandParameters = newChildDescriptors.stream().filter(CommandParameter.class::isInstance).map(CommandParameter.class::cast)
                                .filter(commandParameter -> optionalEClassReference.map(eClassReference -> eClassReference.isInstance(commandParameter.getValue())).orElse(true)).toList();
