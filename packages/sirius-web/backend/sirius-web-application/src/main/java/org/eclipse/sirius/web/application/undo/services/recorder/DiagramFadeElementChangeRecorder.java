...
/**
 * Use to record data needed to perform the undo/redo for FadeDiagramElementEvent.
 *
 * @author mcharfadi
 */
@Service
    ...
    public void accept(IEditingContext editingContext, Diagram previousDiagram, List<IDiagramEvent> diagramEvents, List<ViewDeletionRequest> viewDeletionRequests, List<ViewCreationRequest> viewCreationRequests, ChangeDescription changeDescription) {
        if (editingContext instanceof EditingContext siriusEditingContext && changeDescription.getInput() instanceof IDiagramInput diagramInput) {
            ...
            diagramEvents.stream()
                    .filter(FadeDiagramElementEvent.class::isInstance)
                    .map(FadeDiagramElementEvent.class::cast)
                    .forEach(fadeDiagramElementEvent -> {
                        var diagramFadeElementChange = new DiagramFadeElementChange(diagramInput.id(), diagramInput.representationId(), fadeDiagramElementEvent.getElementIds(), !fadeDiagramElementEvent.fadeElement(), fadeDiagramElementEvent.fadeElement());
