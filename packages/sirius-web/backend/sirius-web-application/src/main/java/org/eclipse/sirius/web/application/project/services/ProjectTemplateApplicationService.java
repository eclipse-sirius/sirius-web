...
public class ProjectTemplateApplicationService implements IProjectTemplateApplicationService {
    ...
    private Optional<ProjectTemplate> getUploadProject() {
        Optional<ProjectTemplate> result = Optional.empty();
        var canCreate = this.capabilityVoters.stream().allMatch(voter -> voter.vote(VOTER_PROJECT_TYPE, null, "canUpload") == CapabilityVote.GRANTED);
        if (canCreate) {
    ...
    private Optional<ProjectTemplate> getBrowseAllProjectTemplates() {
        Optional<ProjectTemplate> result = Optional.empty();
        var canCreate = this.capabilityVoters.stream().allMatch(voter -> voter.vote(VOTER_PROJECT_TYPE, null, "canBrowseAllProjectTemplates") == CapabilityVote.GRANTED);
        if (canCreate) {
