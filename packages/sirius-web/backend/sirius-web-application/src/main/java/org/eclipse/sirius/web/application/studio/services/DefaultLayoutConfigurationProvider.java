...
import org.eclipse.sirius.components.collaborative.diagrams.messages.CollaborativeDiagramMessageService;
...
public class DefaultLayoutConfigurationProvider implements ILayoutConfigurationProvider {
    ...
    private final CollaborativeDiagramMessageService messageService;

    public DefaultLayoutConfigurationProvider(CollaborativeDiagramMessageService messageService) {
    ...
    public List<LayoutConfiguration> getLayoutConfiguration(IEditingContext editingContext, DiagramContext diagramContext, DiagramDescription diagramDescription) {
        ...
        LayoutConfiguration layoutSporeOverlap = LayoutConfiguration.newLayoutConfiguration("elk-spore-overlap")
                .label(this.messageService.diagramLayoutOrthogonal())
                .iconURL(List.of(DiagramImageConstants.SPOREOVERLAP_SVG))
                .layoutOptions(getSporeOverlapOptions())
                .build();

        return List.of(layoutLayered, layoutRectPacking, layoutSporeOverlap);
    }
    ...
    private List<LayoutOptionEntry> getLayeredOptions(String direction) {
        return List.of(
                ...
                new LayoutOptionEntry("elk.alignment", "CENTER")
        );
    }
    ...
    private List<LayoutOptionEntry> getSporeOverlapOptions() {
        return List.of(
                new LayoutOptionEntry("elk.algorithm", "sporeOverlap"),
                new LayoutOptionEntry("elk.spacing.nodeNode", "40"),
                new LayoutOptionEntry("elk.nodeSize.fixedGraphSize", "true"),
                new LayoutOptionEntry("elk.underlyingLayoutAlgorithm", "stress")
        );
    }
}