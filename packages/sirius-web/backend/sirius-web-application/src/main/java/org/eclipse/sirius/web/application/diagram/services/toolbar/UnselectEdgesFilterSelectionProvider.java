...
/**
 * Handler used to get the unselect edges menu item of the FilterSelectionMenu in the diagram toolbar.
 *
 * @author mcharfadi
 */
@Service
public class UnselectEdgesFilterSelectionProvider implements IDiagramToolbarFilterSelectionProvider {

    public static final String ID = "unselect_all_edges";

    private final DiagramQueryService diagramQueryService;

    private final ISiriusWebApplicationMessageService messageService;

    public UnselectEdgesFilterSelectionProvider(DiagramQueryService diagramQueryService, ISiriusWebApplicationMessageService messageService) {
    ...
    public boolean canHandle(IEditingContext editingContext, DiagramContext diagramContext, List<String> diagramElementIds) {
        return !diagramElementIds.isEmpty() && diagramElementIds.stream().anyMatch(diagramElementId -> this.diagramQueryService.findEdgeById(diagramContext.diagram(), diagramElementId).isPresent());
    }
    ...
    public FilterSelectionMenuItem getDiagramToolbarFilterSelectionMenuItem(IEditingContext editingContext, DiagramContext diagramContext, List<String> diagramElementIds) {
        return new FilterSelectionMenuItem(ID, this.messageService.filterSelectionProviderUnSelectEdges());
    }
}
