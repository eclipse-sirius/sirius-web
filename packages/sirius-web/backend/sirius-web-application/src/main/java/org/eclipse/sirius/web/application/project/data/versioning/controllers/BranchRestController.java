...
public class BranchRestController {
    ...
    @GetMapping
    public ResponseEntity<List<RestBranch>> getBranches(@PathVariable UUID projectId) {
        var payload = this.editingContextDispatcher.dispatchQuery(projectId.toString(), new GetBranchesRestInput(UUID.randomUUID())).block(Duration.ofSeconds(TIMEOUT));
    ...
    @PostMapping
    public ResponseEntity<RestBranch> createBranch(@PathVariable UUID projectId, @RequestBody RestBranchRequest requestBody) {
        UUID commitId = null;
        var head = requestBody.head();
        if (head != null) {
            commitId = head.id();
        }
        var payload = this.editingContextDispatcher.dispatchMutation(projectId.toString(), new CreateBranchRestInput(UUID.randomUUID(), requestBody.name(), commitId)).block(Duration.ofSeconds(TIMEOUT));
    ...
    public ResponseEntity<RestBranch> getBranchById(@PathVariable UUID projectId, @PathVariable UUID branchId) {
        var payload = this.editingContextDispatcher.dispatchQuery(projectId.toString(), new GetBranchByIdRestInput(UUID.randomUUID(), branchId)).block(Duration.ofSeconds(TIMEOUT));
    ...
    public ResponseEntity<RestBranch> deleteBranch(@PathVariable UUID projectId, @PathVariable UUID branchId) {
        ...
        if (restProject.isPresent()) {
            var payload = this.editingContextDispatcher.dispatchQuery(projectId.toString(), new DeleteBranchRestInput(UUID.randomUUID(), branchId)).block(Duration.ofSeconds(TIMEOUT));
