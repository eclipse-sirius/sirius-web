...
/**
 * Handler to unselect all edges for a given selection in the diagram toolbar.
 *
 * @author mcharfadi
 */
@Service
public class UnselectEdgesFilterSelectionHandler implements IDiagramToolbarFilterSelectionHandler {

    private final IDiagramQueryService diagramQueryService;

    public UnselectEdgesFilterSelectionHandler(IDiagramQueryService diagramQueryService) {
    ...
    public boolean canHandle(IEditingContext editingContext, DiagramContext diagramContext, String toolbarFilterSelectionMenuItemId, List<String> diagramElementIds) {
        return toolbarFilterSelectionMenuItemId.equals(UnselectEdgesFilterSelectionProvider.ID);
    }
    ...
    public List<String> getNewSelection(IEditingContext editingContext, DiagramContext diagramContext, String toolbarFilterSelectionMenuItemId, List<String> diagramElementIds) {
        return diagramElementIds.stream()
                .filter(diagramElementId -> this.diagramQueryService.findNodeById(diagramContext.diagram(), diagramElementId).isPresent())
                .toList();
    }
}
