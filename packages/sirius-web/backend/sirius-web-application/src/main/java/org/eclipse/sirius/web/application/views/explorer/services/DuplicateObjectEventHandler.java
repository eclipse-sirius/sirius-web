...
public class DuplicateObjectEventHandler implements IEditingContextEventHandler {
    ...
    private final IDefaultObjectDuplicator defaultDuplicator;

    private final List<IObjectDuplicator> objectDuplicators;

    private final IMessageService messageService;

    private final Counter counter;

    public DuplicateObjectEventHandler(IObjectSearchService objectSearchService, IDefaultObjectDuplicator defaultDuplicator, List<IObjectDuplicator> objectDuplicators, IMessageService messageService, MeterRegistry meterRegistry) {
        this.objectSearchService = Objects.requireNonNull(objectSearchService);
        this.messageService = Objects.requireNonNull(messageService);
        this.defaultDuplicator = Objects.requireNonNull(defaultDuplicator);
        this.objectDuplicators = Objects.requireNonNull(objectDuplicators);
    ...
    private IStatus duplicateObject(IEditingContext editingContext, String objectToDuplicateId, String containerId, String containmentFeature, DuplicationSettings settings) {
        ...
        Optional<EObject> objectToDuplicateOpt = this.objectSearchService.getObject(editingContext, objectToDuplicateId)
        ...
        if (objectToDuplicateOpt.isEmpty()) {
            ...
            var optionalCustomDuplicator = this.objectDuplicators.stream()
                    ...
                    .findFirst();
            if (optionalCustomDuplicator.isPresent()) {
                result = optionalCustomDuplicator.get().duplicateObject(editingContext, objectToDuplicateOpt.get(), containerEObjectOpt.get(), containmentFeature, settings);
            } else {
                result = this.defaultDuplicator.duplicateObject(editingContext, objectToDuplicateOpt.get(), containerEObjectOpt.get(), containmentFeature, settings);
            }
        }
