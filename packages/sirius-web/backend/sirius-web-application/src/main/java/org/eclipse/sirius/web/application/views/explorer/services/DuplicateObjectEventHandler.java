...
public class DuplicateObjectEventHandler implements IEditingContextEventHandler {
    ...
    public DuplicateObjectEventHandler(IObjectService objectService, ICollaborativeMessageService collaborativeMessageService, IEMFMessageService emfMessageService, IMessageService messageService, MeterRegistry meterRegistry) {
    ...
    public void handle(One<IPayload> payloadSink, Many<ChangeDescription> changeDescriptionSink, IEditingContext editingContext, IInput input) {
        ...
        List<Message> messages = List.of(new Message(this.collaborativeMessageService.invalidInput(input.getClass().getSimpleName(), DuplicateObjectInput.class.getSimpleName()), MessageLevel.ERROR));
    ...
    private IStatus duplicateObject(IEditingContext editingContext, String objectToDuplicateId, String containerId, String containmentFeature, boolean duplicateContent,
        ...
        if (objectToDuplicateOpt.isEmpty()) {
            result = new Failure(this.collaborativeMessageService.objectDoesNotExist(objectToDuplicateId));
        } else if (containerEObjectOpt.isEmpty()) {
            result = new Failure(this.collaborativeMessageService.objectDoesNotExist(containerId));
        } else {
        ...
        if (optionalEditingDomain.isPresent()) {
            ...
            if (eReferenceOpt.isPresent()) {
                if (this.canFeatureBeSet(containerEObject, eReferenceOpt.get())) {
                    ...
                    result = new Failure(this.emfMessageService.upperBoundaryReached(objectToDuplicate.eClass().getName(), containmentFeature));
                }
            }
