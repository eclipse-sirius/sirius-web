...
/**
 * Handler to unselect all nodes for a given selection in the diagram toolbar.
 *
 * @author mcharfadi
 */
@Service
public class UnselectNodesFilterSelectionHandler implements IDiagramToolbarFilterSelectionHandler {
    ...
    public boolean canHandle(IEditingContext editingContext, DiagramContext diagramContext, String toolbarFilterSelectionMenuItemId, List<String> diagramElementIds) {
        return toolbarFilterSelectionMenuItemId.equals(UnselectNodesFilterSelectionProvider.ID);
    }
    ...
    public List<String> getNewSelection(IEditingContext editingContext, DiagramContext diagramContext, String toolbarFilterSelectionMenuItemId, List<String> diagramElementIds) {
        return diagramElementIds.stream()
                .filter(diagramElementId -> this.diagramQueryService.findEdgeById(diagramContext.diagram(), diagramElementId).isPresent())
                .toList();
    }
}
