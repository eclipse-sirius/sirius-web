...
/**
 * Used to handle the undo/redo for the ResetViewModifiersEvent.
 *
 * @author mcharfadi
 */
@Service
public class ResetViewModifiersChangeHandler implements IRepresentationChangeHandler {

    private final IRepresentationEventProcessorRegistry representationEventProcessorRegistry;

    public ResetViewModifiersChangeHandler(IRepresentationEventProcessorRegistry representationEventProcessorRegistry) {
    ...
    public boolean canHandle(UUID inputId, IEditingContext editingContext) {
        return editingContext instanceof EditingContext siriusEditingContext && siriusEditingContext.getInputId2RepresentationChanges().get(inputId) != null &&
                ...
                .anyMatch(ResetViewModifiersChange.class::isInstance);
    }
    ...
    public void redo(UUID inputId, IEditingContext editingContext) {
        if (editingContext instanceof EditingContext siriusEditingContext) {
            siriusEditingContext.getInputId2RepresentationChanges().get(inputId).stream().filter(ResetViewModifiersChange.class::isInstance)
                    .map(ResetViewModifiersChange.class::cast)
                    .forEach(change -> {
                        ...
                        if (representationEventProcessorEntry != null && representationEventProcessorEntry.getRepresentationEventProcessor() instanceof DiagramEventProcessor eventProcessor) {
                            ...
                            diagramContext.diagramEvents().add(new ResetViewModifiersEvent(change.elementIds()));
                        }
                    });
        }
    }
