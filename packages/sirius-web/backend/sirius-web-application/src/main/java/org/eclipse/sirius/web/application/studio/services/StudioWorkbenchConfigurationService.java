 ...
 */
@Service
public class StudioWorkbenchConfigurationService implements IWorkbenchConfigurationServiceDelegate {

    private final IStudioCapableEditingContextPredicate studioCapableEditingContextPredicate;

    private final IRepresentationMetadataSearchService representationMetadataSearchService;

    public StudioWorkbenchConfigurationService(IStudioCapableEditingContextPredicate studioCapableEditingContextPredicate, IRepresentationMetadataSearchService representationMetadataSearchService) {
    ...
    public WorkbenchConfiguration getWorkbenchConfiguration(String editingContextId) {
        List<WorkbenchRepresentationEditorConfiguration> representationToOpen = new ArrayList<>();
        new UUIDParser().parse(editingContextId)
                .stream()
                .flatMap(id -> this.representationMetadataSearchService.findAllRepresentationMetadataBySemanticData(AggregateReference.to(id)).stream())
                .filter(representationMetadata -> "Domain".equals(representationMetadata.getLabel()))
                .map(RepresentationMetadata::getId)
                .findFirst()
                .ifPresent(representationId -> {
                    representationToOpen.add(new WorkbenchRepresentationEditorConfiguration(representationId.toString(), true));
                });

        return new WorkbenchConfiguration(new WorkbenchMainPanelConfiguration("main", representationToOpen), List.of(
