...
public class RepresentationIdBuilder {
    ...
    public String buildViewsViewRepresentationId(String treeDescriptionId, List<String> expandedObjectIds) {
        List<String> encodedExpandedObjectIds = expandedObjectIds.stream()
                .map(id -> URLEncoder.encode(id, StandardCharsets.UTF_8))
                .toList();

        return "views://?treeDescriptionId=" + URLEncoder.encode(treeDescriptionId, StandardCharsets.UTF_8) + EXPANDED_IDS + String.join(",", encodedExpandedObjectIds) + "]";
    }
