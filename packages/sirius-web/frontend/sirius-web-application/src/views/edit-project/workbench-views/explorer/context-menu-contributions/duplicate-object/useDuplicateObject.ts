...
export const useDuplicateObject = (): UseDuplicateObjectValue => {
  const [mutationDuplicateObject, mutationDuplicateObjectResult] = useMutation<
    GQLDuplicateObjectData,
    GQLDuplicateObjectVariables
  >(duplicateObjectMutation);
  useReporting(mutationDuplicateObjectResult, (data: GQLDuplicateObjectData) => {
  ...
  if (mutationDuplicateObjectResult?.data?.duplicateObject?.__typename === 'DuplicateObjectSuccessPayload') {
    duplicatedObject = mutationDuplicateObjectResult.data.duplicateObject.object;
  }

  return { duplicateObject, duplicatedObject, loading: mutationDuplicateObjectResult?.loading };
};
