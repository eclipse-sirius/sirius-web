...
export const DuplicateObjectKeyboardShortcut = ({
  ...
  useEffect(() => {
    if (!readOnly && target) {
      target.addEventListener('keydown', duplicateObjectKeyPressHandler);
    }
    return () => {
      target?.removeEventListener('keydown', duplicateObjectKeyPressHandler);
    };
  }, [selectedTreeItem, readOnly, target]);
