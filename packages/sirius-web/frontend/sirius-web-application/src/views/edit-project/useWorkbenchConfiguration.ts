...
export const useWorkbenchConfiguration = (editingContextId: string | null): UseWorkbenchConfigurationValue => {
  const { addErrorMessage } = useMultiToast();
  const { data, error } = useQuery<GQLGetWorkbenchConfigurationData, GQLGetWorkbenchConfigurationVariables>(
    getWorkbenchConfigurationQuery,
    {
      variables: {
        editingContextId,
      },
      skip: !editingContextId,
    }
  );
