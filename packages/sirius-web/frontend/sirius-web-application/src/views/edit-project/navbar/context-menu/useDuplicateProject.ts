...
function isDuplicateSuccessPayload(payload: any): payload is GQLDuplicateSuccessPayload {
...
export const useDuplicateProject = (): UseDuplicateProjectValue => {
  const [performProjectDuplicate, { loading, data, error }] = useMutation<
    GQLDuplicateProjectMutationData,
    GQLDuplicateProjectMutationVariables
  ...
  const duplicateProject = (projectId: string) => {
    ...
    performProjectDuplicate({ variables });
  };

  const newProjectId = isDuplicateSuccessPayload(data?.duplicateProject) ? data.duplicateProject.newProject.id : null;
  return {
    duplicateProject,
    loading,
    newProjectId,
