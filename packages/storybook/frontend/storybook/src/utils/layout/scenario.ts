 ...
 *******************************************************************************/
import { GQLDiagram, GQLDiagramDescription } from '@eclipse-sirius/sirius-components-diagrams';
...
interface ScenarioData {
  ...
  representations: Record<string, GQLDiagram>;
}
...
Object.entries(allJsonFiles).forEach(([path, data]) => {
  ...
  if (layoutIndex !== -1 && parts.length >= layoutIndex + 4) {
    ...
    if (folderType === 'descriptions') {
      groupedScenarios[diagramType].description = data as GQLDiagramDescription;
    } else if (folderType === 'representations') {
      groupedScenarios[diagramType].representations[scenarioName] = data as GQLDiagram;
    }
  }
});
    ...
    if (data.description && data.representations) {
      for (const [scenarioName, representationData] of Object.entries(data.representations)) {
        ...
        scenarioGenerators[uniqueKey] = () => {
          return assembleDiagram(representationData, data.description!);
        };
      }
    }
  }
