...
export const ModelBrowserTreeView = ({
  ...
  const treeId: string = useMemo(() => {
    return `modelBrowser://${modelBrowserId}?ownerKind=${encodeURIComponent(ownerKind)}&targetType=${encodeURIComponent(
      referenceKind
    )}&ownerId=${ownerId}&descriptionId=${encodeURIComponent(descriptionId)}&isContainment=${isContainment}`;
  }, [descriptionId, isContainment, modelBrowserId, ownerId, ownerKind, referenceKind]);
